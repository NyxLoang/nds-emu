...
| 2026-09-13 | 21-B9yi（续62，性能·修正+收益） | 工程/构建 | **LTO 其实从未生效**：`check_ipo_supported()` 默认按 CXX 探测，而工程 `project(... C)` 只有 C ⇒ 一直失败，`build.ninja` 里只有 `-std=c11 -O3`。改为 `LANGUAGES C` 后出现 `-flto=auto`：同机相邻配对 **1000 帧 12.35→10.72 s（-13.2%，帧率 +15%）**、**10000 帧（含重场景）122.2→109.4 s（-10.5%）**。并更正续41 的旧记录（当时把 LTO 记为「零收益」，实为两个构建都没开 LTO）。零回归：931/0、1000 帧时间线逐行同、2000 帧统计逐值同 + 截图 SHA-256 同 | [cpu](src/cpu/cpulog.md) · [runner](src/runner/runnerlog.md) · [21](docs/21-rom-bringup.md) |
