...
| 维度 | 状态 |
|------|------|
| 画面 | 默认帧节奏对齐真机 59.83 fps，并允许「欠账追帧」⇒ 重场景掉到 ~52 fps 后会在轻场景补回来，**长时间游玩音画不漂移**（12000 帧实测漂移 ≈0.1 s）；**满速能力 52–265 fps**（重/轻场景，`NDS_NOSYNC=1` 可看）；渲染改动**逐像素零回归**（截图 SHA-256 不变） |
| 上手/诊断 | `--help` 打印按键表 + 开关表 + 诊断环境变量；窗口启动打印按键提示；`--fps-every` 给出 `phases/render/pace/drift` 四行量化 |
| 键盘 | 全部按键可用（多轮长跑） |
...
> 快进（按住 `Tab`）的**实际倍率受模拟速度限制**：上限 ×4，但 1× 只有 ~70 fps 的重场景
> 快进时也只能到 ~67 fps（≈1.1×）；轻场景（1× 时 150–265 fps）才能真正跑满 ×4。

> `--fps-every` 输出里另有 `pace:` 一行：统计每帧超出目标截止时刻的量与「丢帧重同步」次数，
> 用来判断帧率不足是**节奏实现**问题还是**重场景突发负载**问题（实测轻场景 0.21 ms/帧，
> 重场景每 3000 帧有 27–35 个超出一整帧预算的突发帧）。
...
> （PC/寄存器/CPSR/**IF 位**/调度时间/ARM7 定时器状态），用于 diff 两条本该一致的时间线
> （例如「存档后继续」与「读档后继续」）；`NDS_STATEDBG=1` 打印存档/读档瞬间的中断与定时器状态。

> `--fps-every` 还会打印 `drift:` 一行：这一段「模拟时间 vs 墙钟」的差（正值=画面落后、声音超前）。
> 帧节奏默认允许最多 30 s 的「欠账追帧」，让模拟时间跟住墙钟 ⇒ 音画长时间不漂移；
> `NDS_PACE_DEBT_MS=N` 可改上限（100 可复现旧的「不追帧」行为）。

> 忘了按键或开关？直接 `build\nds-emu.exe --help`：里面有按键表、常用开关与诊断环境变量
> 的完整清单（窗口启动时也会打印一行按键提示）。
