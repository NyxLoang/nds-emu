...
### 7.1 21-B9yd/B9ye：从「可读字节影子」升级为「完整镜像 + 真地址执行」

上面第 7 节只把**会被读的少量字节**（向量表、SWI 表）影子化，其余低地址读 0；
未被逐条建模的 SWI 函数体是在分发器里调用 C 版 HLE 算完结果、直接跳到
`swi_complete`。结果虽然对，但 **BIOS 内部 PC/lr/栈帧轨迹与参考核不同**，
于是「IRQ 落在函数体内部」时保存/恢复的任务现场也就不一样——这正是
FFXII 的 ARM7 协作式调度器最敏感的地方。

21-B9yd 起改成两条腿走路：

- `src/bios/bios7_rom.h` 内嵌**完整 0x4000 字节 FreeBIOS ARM7 镜像**
  （BSD-2，来源同 melonDS；有效长度 0x2030，其余按参考核口径补 0），
  低地址任意一次读都返回参考核的字节；
- `bios7_low.c` 只逐条等价实现「与硬件交互」的部分（SWI 分发器、Halt/
  IntrWait/WaitByLoop、IRQ 出入口、SoftReset），**其余函数体（Div/CpuSet/
  GetCRC16/LZ77/音频查表…）直接在真地址上执行真实字节**。

21-B9ye 再把 **SWI 分发器 0x1080-0x10A8 逐条拆开**（此前是整段合成一步），
这样 `msr cpsr_fc` 之后（I 位取自调用方 SPSR）IRQ 能像参考核一样插在
0x109C-0x10A8 之间。

**验证方式（参考级证据）**：给两边核心都加「低地址取指直方图」——本地
`NDS_BIOS7_HIST=1`（可选 `NDS_BIOS7_HIST_OUT=<path>` 存 4096×u32），
参考核 dump `hist7: pc=… count=…`；跑同样帧数后逐地址比对。
900 帧结果：**两边各命中 93 个低地址字，集合完全相同**（差异为空集），
IRQ 路径计数差 0.3%、CRC16 循环 112 次完全一致。
