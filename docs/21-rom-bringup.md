...
- 参考同一时刻在 ITCM 切到 0x76FE4 后直接发下一条 1AB/22B，没有
  再次切回 0x76F24；唤醒瞬间 `[0x76FE4+0x64]` 两侧都是 1，差异在
  ITCM 切换之后、下一次调度之前是否又被 FIFO 分发循环“吃掉”。

下一步方向：第二次 6B 之后本地 0x0200B65C 会连续读 20 余条 C024 才退出，
参考在该时刻没有积压队列；优先对比 FIFO 队列积压与 C024 回执节奏。
