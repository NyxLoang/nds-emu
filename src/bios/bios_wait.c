...
/* SWI 0x06 Halt（ARM9）。
   NDS9 没有 HALTCNT 寄存器，空闲任务用 CP15 WFI 实现（见 cpu_step）；
   这里作为兜底：等 (IE & IF) != 0，未满足就停在 SWI 上重跑（等价忙等）。
   ARM7 的 Halt 已按 FreeBIOS 低地址路径实现（写 HALTCNT=0x80、暂停在
   0x1158、由 IF&IE 唤醒），见 bios7_low.c 与 21-B9wt。 */
...
{
    /* ARM9：按空操作处理（精确时序不影响 ARM9 逻辑）。
       ARM7 的 SWI 3 由 bios7_low.c 逐条执行 0x115C 的 subs/bgt 循环，
       每轮约 3 周期；FFXII 在 0x037FE408 队列等待处正是用这个循环拖时间。 */
    return BIOS_RET_HANDLED;
}
