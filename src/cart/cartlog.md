...
### 21-B9yi（续13）— 一次多余的完成中断（1904 帧停摆的真正根因）

**定位**：把窗口内 931 条命令 / 全部 `END` 事件按形态统计，发现**唯一一条
「没有传输在跑」的结束事件**：

```
f=1888  END pos=0 len=0 cnt=0 romctrl=21416657     ← xfer_len==0
其余 794 条：END pos=512（块读） / 136 条 END pos=4（B8 芯片 ID）——都是真结束
```

游戏的任务计数每个卡带完成中断减 `0x200`（`0x02011B40 subs r1,#0x200`），
多出来的这一次让它**提前一块**以为整段搬完 ⇒ 去读芯片 ID（等 ROMCTRL bit31 落 0），
而最后一块刚发起、数据没人读 ⇒ bit31 恒 1 ⇒ ARM9 停在 `0x02011C50`、
IPC FIFO 积压 16 条、ARM7 被拖进 IRQ 模式。

**为什么会多读一次 `CARD_DATA`**：游戏每块收尾的「停 DMA 助手」`0x020096CC` 先写
`CNT_H=0x8500`（模式 0 = 立即、CNT_L=1）再写 `0x0500`。本地把立即模式搬运
**同步执行在寄存器写里**，这一读可能发生在命令发起之前（`xfer_len==0`）
⇒ `cartbus_read32()` 的 `pos>=len`（0>=0）分支误判为「传输结束」并挂中断。
参考核的搬运是延迟执行的（`Start()` 只置 `Running=2`，真正搬在 `Run9()`），
等它读的时候 `xfer_len=512` ⇒ 不误判。

**修正**：`cartbus_end()` 增加 `real_end = (cb->xfer_len != 0)` 判据，
**只有真在传输中才挂完成中断**（状态清理照旧）。

**结果**（带按键时序）：f=1910 ARM9 回到空闲任务 `0200957C`/`cpsr=0000001F`
（改前同帧是 `02011C50` IRQ 模式死等）；DISPCNT 走完参考核同序列
（80111418 → 80111F18 → 00161F10/00121F10，本地滞后 ~300 帧）；
f=2400 起 GX 出三角形（tri=8906 → 13359，改前恒 0）。

## 2026-09-13 · 21-B9yi（续77）：存档芯片状态诊断（headless 汇总新增 `savechip:` 一行）

- **做了什么**：`runner_print_savechip()` —— 打印
  `savechip: type=… size=… nonzero(vs 0xFF)=… hash=…`。擦除态是 0xFF，所以
  「非 0xFF 字节数 + 内容哈希」能直接回答**「游戏运行中有没有真的写过存档」**
  （此前这项只能靠人工进菜单存档确认）。
- **实测**：
  ```
  短跑（300 帧）：type=2(EEPROM_8K) size=8192 nonzero=24 hash=3A361368EF684AD7
  自然输入 20000 帧（已跑到过场、vram-nz=393216 整块 VRAM）后：**同样的 24 与同样的哈希**
  磁盘 .sav：8192 字节 / 24 个非 0xFF 字节，开头 ASCII **"BLIZ"**（Square 系存档签名）
  ```
- **结论**：**存档头**（24 字节，含 "BLIZ"）曾被写入 ⇒ 存档**写路径**部分得到证明；
  **完整游戏存档**（需玩家进菜单保存）在自动脚本下未被触发。诊断保留：
  将来任何一次真存档都会让 `nonzero` 从 24 明显增大，一眼可判。

## 2026-09-13 · 21-B9yi（续80）：密集连续交互仍不能自动触发存档（负结果）

- **做法**：把输入密度提到「几乎一直有输入」再试两组（不同种子）：
  `--key-random … --key-period 20` + `--touch-random … --touch-period 30`，各 30000 帧
  （约 8 分钟游戏时间；两组分别跑出 43 / 44 张不同画面）。
- **结果**：**`savechip` 完全不变** —— 仍是 `type=2 size=8192 nonzero(vs 0xFF)=24
  hash=3A361368EF684AD7`，与游玩前一致 ⇒ 没有发生真实存档。
- **结论**：游戏的存档需要**刻意走菜单**（开菜单→选存档→确认），随机/密集输入碰不到。
  「游戏内存档」保持**人工确认**；判定手段已经就位（跑完看 `savechip:` 行的
  `nonzero(vs 0xFF)`，只要从 24 明显增大就说明存上了）。
