...
### 续109b：Thumb `BLX` 目标必须**字对齐**（清 bit0/bit1）—— 第二款 ROM 的「未定义指令」根因

- **症状**（Phase C：口袋妖怪 黑2）：本地 ARM9 在引导期进**未定义指令**异常，
  `cpsr=0x1B`（Undefined 模式）后停在 BIOS 空循环、画面全白；参考核同帧正常。
- **现场**：`cpu: PC=0207BE64 insn=1C04B5F8 undefined` —— 该 4 字节是 **Thumb 码**
  （`push {r3-r7,lr}` + `add r4,r0,#0`），说明 CPU 该在 Thumb 态却按 ARM 译码。
  trace 回溯到切换点：Thumb 代码 `BLX` 后 PC 落在 **`0x0207BDC6`**（2 mod 4），
  参考核落在 **`0x0207BDC4`**（字对齐）。
- **根因**：Thumb `BLX <label>`（格式 18 第二半字 `11101`）的语义是
  **目标按字对齐**（H 位丢弃，目标是 ARM 指令）；本地写成 `target & ~1u`，
  只清 bit0。当 `LR + (imm11<<1)` 的 **bit1 为 1** 时目标偏 +2 ⇒ 在非字对齐地址
  按 ARM 译码 ⇒ 取到非法/未定义指令。
- **改法**：`thumb.c` 的 BLX 分支改成 `target = (cpu->r[14] + ((insn & 0x7FFu) << 1)) & ~3u`
  （`BL`（`11111`）保持 Thumb、不需要字对齐，原样不动）。
- **验证**：单测 994 → **997 项 0 失败**（新增「BLX 原始和 = base+0x22（bit1=1）⇒ PC 必须
  是 base+0x20」；原有 BLX 用例恰好字对齐，因此此前没暴露）。端到端：该 ROM 不再进
  未定义异常，ARM9 继续执行（PC 进入 `0x0207C6xx` 系列的真实引导码）；FFXII 锚点
  2000 帧三项逐值不变。

### 续109c：Thumb `BL`/`BLX` 的返回地址偏 +2（跳过一条指令）

- **症状**：续109b 之后该 ROM 卡在 `0x0207C6A4` 的「写寄存器 → 读回等待变化」循环；
  trace 显示循环指针 **r0 = 0**（应为 `0x04000180` IPCSYNC），而 `r5` 一路正确
  （字面量池 `ldr r5,[pc,#0x6c]` → `0x04000184`）⇒ 唯一解释是 `subs r0,r5,#4`
  （`0x0207C67E`）**从未执行**。
- **取证**：该处 BL（`0x0207C67A`，`F7FD FA69` → `0x02079B50`）返回后 **PC=0x0207C672、
  LR=0x0207C673** ⇒ 返回地址 = BL 地址 + 6，正好跳过 `0x0207C67E` 那条 2 字节指令。
- **根因**：BL 第二半字应把 LR 写成「BL 之后那条指令」= 第二半字地址 S + 2 = `pc - 2`
  （`pc` 是本模拟器的流水线值 S+4），本地写成了 `pc | 1`。参考核
  `ARMInterpreter_Branch.cpp::T_BL_LONG_2` 为 `R[14] = (R[15] - 2) | 1`（R[15] = S+4），
  本地 ARM 侧 `r[14] = r[15] + 4` 也符合「下一条指令」口径 ⇒ Thumb 侧是孤例。
- **改法**：BL 与 BLX 分支统一改成 `cpu->r[14] = ((pc - 2u) | 1u)`。
- **验证**：单测 **997 项 0 失败**（两条旧断言 `(base+6)|1` **按规范改成 `(base+4)|1`** ——
  它们此前把 bug 固化成了期望值）；FFXII 锚点 2000 帧三项逐值不变；该 ROM 的 IPCSYNC
  空转消失，推进到 ITCM `0x01FF803C`（**下一处待查**：那里又在 ARM 态执行 Thumb 字节
  ⇒ 还有一处「T 位丢失」路径）。

### 续109d：T 位变化定点钩子（`NDS_TWATCH=1`）+ Thumb `BLX Rm` 的 LR 同一条 +2 修正

- **新增诊断**：`cpu.c` 里加 `twatch_check()`（`NDS_TWATCH=1` 时打印**哪条指令改了
  CPSR.T**：核心号/帧号/PC/指令字/新旧 T/CPSR/r0-r2/LR/SP）。零开销：默认只多一次
  整数比较。它把「ARM 态跑 Thumb 码」这类问题从「猜」变成「看一行」。
- **用它拿到的第一条结论**（口袋妖怪 黑2，f=11 一共只有 6 次 T 变化）：

```text
f=11 pc=020049A8 insn=E12FFF11 T 0->1   BX r1（r1=0x02005125 奇）✓ 正常
f=11 pc=0207C638 insn=EBC6     T 1->0   BLX 立即数 → ARM ✓ 正常
f=11 pc=0207BDD4 insn=E12FFF1E T 0->1   BX lr（lr=…C63B 奇）✓ 正常
f=11 pc=0207C6BE insn=EB8C     T 1->0   BLX 立即数 → ARM ✓ 正常
f=11 pc=0207BDEC insn=E12FFF1E T 0->1   BX lr ✓ 正常
f=11 pc=0207C464 insn=4790     T 1->0   **BLX r2，而 r2 = 0** ← 问题在这里
```

  即：不是「T 位被莫名清掉」，而是游戏**调了一个值为 0 的函数指针**（`BLX r2`, r2=0）
  ⇒ 跳到地址 0（ARM 态）⇒ 一路执行未映射区的 0（ARM NOP）直到 ITCM `0x01FF8000`
  才有非零字节 ⇒ 把 Thumb 码当 ARM 码执行 ⇒ `PC=01FF803C` 未定义指令。**所以
  `0x01FF803C` 是症状，不是根因。**
- **顺手修掉同一族 bug**：`BLX Rm` 的 LR 也写成 `pc | 1`（+2）。参考核 `T_BLX_REG`
  是 `lr = R[15] - 1`（R[15] = pc+4）⇒ 同样应等价于「下一条指令」| 1。改法与 BL/BLX
  立即数一致：`cpu->r[14] = ((pc - 2u) | 1u)`。
- **验证**：单测 997 → **1003 项 0 失败**（新增 2 组 `BLX Rm`：奇数目标 → T=1、
  偶数目标 → T=0，两组都断言 LR = `(base+2)|1`）；FFXII 锚点 2000 帧三项逐值不变。
- **下一步（留给下一轮）**：查「为什么 r2 是 0」——现场是 Thumb 跳转表分发
  （`0x0207B05C add r0,PC` → `0x0207B05E ldrh r0,[r0,#6]` → `0x0207B060 lsls r0,r0,#16`
  → `0x0207B062 add pc,r0`）之后落到 `0x0207C464 BLX r2`，即**表里取到 0**。
  下一步沿「这张表是谁填的/该由谁填」查（对照参考核同址内存）。

### 续109e：Thumb 把 **r15 当操作数读**时必须给「指令地址 + 4」

- **症状**：续109d 之后 ARM9 不再死锁，但落点仍然偏（`BLX r2` 而 r2=0）。逐条对现场
  发现跳转表分发的那 5 条指令里，`ADD r0, PC`（`0x4478`）算出来的基址**比应有值小 4**。
- **根因**：本模拟器的 `r[15]` 存的是**指令地址**（流水线值放在 `thumb_step` 的局部变量
  `pc` 里），而 ARM 架构规定「把 r15 当普通操作数读」时读到的是 **PC = 指令地址 + 4**
  （ARM 侧早就用 `read_reg(cpu,15)` 统一 +8 修正过；Thumb 侧漏了）。受影响的是格式 5
  的高寄存器形式：`ADD/CMP/MOV Rd, PC` 以及 `ADD PC, Rm`、`BX/BLX Rm`（Rm=15 时）。
- **改法**（`thumb.c`）：格式 5 里凡是读取操作数寄存器的地方，`rm/rd/rx == 15` 时改用
  `pc & ~1`；写 PC 时按规范 `PC = Rm & ~1`（MOV）或 `PC = (PC+4)&~1 + Rm`（ADD）。
- **验证**：单测 1003 → **1006 项 0 失败**（新增 `ADD r0,PC`、`MOV r0,PC`、`ADD PC,r0`）；
  FFXII 锚点 2000 帧三项逐值不变、**20000 帧自然输入长跑也与基线逐值一致**；
  Pokemon 的未定义指令消失（暴露下一处，见续109g 结尾）。

### 续109f：ARMv5T **互调加载** —— `LDR pc,[…]` / `LDM …,{pc}` 按 bit0 切状态

- **症状**：续109e 之后 ARM9 在 `PC=0207AFA3`（**奇数**）报未定义指令、且 `cpsr` 仍是
  ARM 态 —— 那 4 字节是标准 Thumb 码（`0x9000` = `str r0,[sp]`）。
- **根因**：ARMv5T 里「从内存加载进 PC」是**互调加载**：值的 bit0 决定之后的状态
  （1=Thumb）且不进 PC；本地直接把内存值原样写进 `r[15]` ⇒ 从 ARM 代码返回 Thumb 代码
  （bit0=1 的函数指针/返回地址，ARMv5T 的常见习惯）时 PC 变奇数、状态仍是 ARM ⇒ 把
  Thumb 字节当 ARM 码译码。参考核 melonDS：`if (cpu->Num==1) val &= ~0x1; JumpTo(val);`
  （`JumpTo` 按 bit0 设/清 T；ARM7 不做互调）。
- **改法**：新增 `arm_write_pc_load(cpu,val)`（ARM9：按 bit0 设/清 CPSR.T、PC=val&~1；
  ARM7：只清 bit0、状态不变），在 `exec_single_transfer`（LDR rd=15）与
  `exec_block_transfer`（LDM 列表含 PC 且**非**异常返回 `^`）里使用；
  `LDM … ^`（异常返回）保持原口径（PC 取加载值、状态由 SPSR 决定）。
- **验证**：单测 1006 → **1012 项 0 失败**（ARM9 的 LDR pc / LDM {pc} 互调、ARM7 不互调）；
  FFXII 锚点与 20000 帧长跑均逐值不变；Pokemon 未定义指令消失。

### 续109h：PC 入口定点钩子（`NDS_PCWATCH`）+ 找到该 ROM 的真正卡点：**IPC 握手的节奏**

- **新增诊断**：`NDS_PCWATCH=ADDR[,ADDR...]`（最多 8 个，16 进制）。PC 命中被监视地址时
  打印**上一条执行过的指令**（`from=` + `insn=`）与当前寄存器/LR/SP —— 直接回答
  「控制流是从哪儿跳进来的」。默认关闭。
- **它纠正了上一轮的猜测**：上一轮以为「CPU 从函数中间跳进等待循环、没跑序言」，
  实测是**正常穿过序言**（`r3=0xF`、`r0=0x04000180`、`lr=0x0207C67F`）：

```text
pcwatch: arm9 f=11 entered=0207C686 from=0207C684 insn=0000250F  r3=0F r5=0F
pcwatch: arm9 f=11 entered=0207C6A4 from=0207C6A2 insn=0000D108  r1=3E8(超时=1000) r2=07
```

- **计数（60 帧内命中次数）**：`0207C692`（写 IPCSYNC）**3913**、`0207C6A8`（超时路径 →
  `movs r4,#0`）**3903**、`0207C6B6`（轮次 +1）3911、`0207C6BA`（**退出**）**2**。
  ⇒ ARM9 每一轮都在**超时**，而超时会把重试计数 `r4` 清零，于是退出条件
  `r2 == 0 && r4 > 4` 永远不成立 ⇒ 死循环。
- **两核节奏对照**（同一次 trace 的 `now=` 系统时间）：ARM7 的 IPCSYNC 写周期稳定在
  **803 单位**；ARM9 的整轮（写 + 回读 + 1000 次自旋超时）≈ **8600 单位**（60 帧 3900 轮）。
  ⇒ ARM7 一轮的时间比 ARM9 的超时窗口**短约 10 倍**，ARM7 的整个降序序列
  （7→6→…→0，共 8 步）在 ARM9 **第一次自旋还没结束**时就跑完了；等 ARM9 醒来，
  对端 nibble 已经停在 0 不再变化 ⇒ 每轮超时 ⇒ 卡死。
- **参考核对照**：同一 ROM 在参考核里 f=14 就有 IPC FIFO 消息（`fifolog`），说明
  参考核的 ARM9 **顺利穿过**了这个握手；本地 60 帧内 FIFO 写次数 = **0**。
- **结论/下一步**：这不是「语义」缺陷，而是**两核相对节奏**缺陷 —— 具体嫌疑是 ARM7 侧
  的延时例程（`bl 0x037FEFC0`，内部走 BIOS 0x115C 的倒计时循环）在本地被**算得太快**
  （803 单位/轮），而参考核（melonDS）对 ARM7 也是逐条指令计时。下一步：
  ① 把 ARM7 这条延时链的耗用与参考核同口径量出来（ARM7 指令数/周期）；
  ② 检查本地 `bios7_low` 里那段 BIOS 等待循环的计费（是否每步只算 1 单位、丢掉访存等待）；
  ③ 必要时按参考核口径给 ARM7 补同样的访存代价模型（现在 `NDS_MEMTIM` 只作用于 ARM9）。
  注意：**`NDS_MEMTIM=1` 不能修复此问题**（实测该 ROM 仍停在同址）——因为它只改 ARM9 侧。

### 续109i：ARM7 的每帧指令数对照（含一次**计数口径陷阱**的更正）

用两边同口径的每帧指令数（本地 `NDS_INSTRSTAT=5` / 参考核 `REF_INSTRSTAT=5`，同一 ROM、
同一起点）量开机段：

```text
f=10   本地   i9=5,601,900   i7=   150,126
       参考核 i9=5,319,083   i7= 3,805,990   （其中 b7=3,654,040 = **96% 在 ARM7 BIOS 区**
                                             b9=0 → 参考核 ARM9 侧不跑 BIOS）
```

**第一次读数（错的）**：本地 `i7=150,126` ⇒ 看起来「ARM7 只有参考核的 1/25，
说明本地把 BIOS 等待算得太省」。

**更正（21-B9yi 续109j）**：那个数字是**计数口径陷阱** —— 本模拟器的低地址 BIOS 是
「逐条地址建模执行」（`bios7_low_step()`），而这条路径**提前 return、没有累加
`cpu->cycles`**；`NDS_INSTRSTAT` 正好读的是 `cpu->cycles` ⇒ 它把 ARM7 在 BIOS 等待循环里
的绝大部分指令都漏掉了。（`cpu->cycles` 只用于诊断，调度用的是 `step_cycles`，
所以修口径不会改变行为。）给两个「逐条建模」的提前返回点补上计数后复测：
...
⇒ **两核的每帧指令吞吐都已经对齐**（ARM9 差 5%，ARM7 差 1.2%）。参考核 ARM7 的 3.65M/10 帧
几乎全在 BIOS 区（`b7`），本地同样如此 —— 差别不在「跑了多少条」，而在**跑在哪段**：

```text
本地 hot7：0000115C / 00001160（各 46.9%/46.7%）= SWI 0x03 WaitByLoop 的 subs/bgt 循环
参考核 hot7：00001168（83.2%）               = interrupt_check 子程序（IntrWait 的检查循环）
```

即：**本地 ARM7 还卡在 IPCSYNC 握手的延时循环里空转，参考核的 ARM7 早已走完握手、
进入到 IntrWait 等中断**。⇒ 差别是**行为/时序位置**，不是吞吐量。
（这也再次否定「本地把 ARM7 等待算得太省」这条假设，与续109h 的结论一致：
问题在握手的自旋/超时节奏。）

**下一步**：把「ARM7 的一轮握手」（写 IPCSYNC → `bl 0x037FEFC0` 延时 → 回读）与
「ARM9 的一轮」（写 → 回读 → 1000 次自旋）在**参考核**里的耗时量出来（给参考核加
ARM7 侧 IPCSYNC 写日志 + 周期戳），对齐两边后复测；验收判据不变：
FIFO 写次数 0 → >0、`disp` 0 → 非 0。

### 续109k：给参考核的写监视加**周期戳** —— 两核「一轮」都量出来了

**工具**：参考核 harness 的 `REF_WATCH_LO/HI` 日志新增 `t9=/t7=`（melonDS 的
ARM9/ARM7 时间戳，**同一系统时间轴**，单位 = ARM9 周期）；本地对应的是 `--watch`/trace
里的 `now=`。两边因此可以逐轮对齐比较。

**实测（同一 ROM、同一段握手，单位 = 系统单位）**：

```text
参考核：ARM9 一轮 ≈ 1627（写 IPCSYNC 的相邻时间戳差）
         ARM7 一轮 ≈ 1650
        ⇒ 两核**锁步**（各自在等对方），这是握手能收敛的关键
本地  ：握手那段 ARM9 一轮 ≈ 800、ARM7 一轮 ≈ 803（**同样锁步**，但整体快约 2 倍）
        握手序列之后：ARM9 变成 ~7000–8600 一轮 ⇒ 说明它每一轮都在**超时**
```

**方向性实验（新增诊断开关 `NDS_ARM7_MUL=N`，默认 1 = 零改动）**：把 ARM7 每步周期
乘以 2、3 后再跑该 ROM —— **握手依然不通过**（FIFO 写仍为 0、`disp` 仍为 0）
⇒ 「ARM7 算得太快」**不是**充分解释，不能只靠放慢 ARM7 修好它。

**新的现场（运行期真码，来自 trace 的 `inst=` 字段）**：卡住的循环是

```text
0x0207C6AC  ldrh r6,[r0]      ; r0 = 0x00000080（ITCM 里的信箱变量）
0x0207C6AE  subs r1,r1,#1
0x0207C6B0  ands r6,r3        ; r3 = 0  ← 掩码为 0
0x0207C6B2  cmp  r6,r2        ; r2 = 0
0x0207C6B4  beq  0x0207C6A4   ; 恒等 ⇒ 只能靠超时退出，而超时又把 r4 清零
```

注意 `pcwatch` 显示这一次调用**不是**从 `0x0207C660` 序言进来的（f=11 那次序言把
`r5=0x04000184 → r0=0x04000180` 指向 IPCSYNC；现在 `r0=0x80` ⇒ 同一个「通用同步例程」
被用在了**另一块内存（ITCM 信箱）**上）。⇒ 下一步：查**谁**用 `r0=0x80` 调它、
以及该变量本应由谁改写（对照参考核同址/同帧的值），而不是继续调时序。
