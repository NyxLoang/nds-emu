...
### 续109i：ARM7 的 BIOS/等待时间只有参考核的 **1/25** —— 节奏缺陷的定量证据
...
⇒ ① **ARM9 两边只差 5%**（与 FFXII 的结论一致：ARM9 侧节奏已对齐）；
② **ARM7 本地只有参考核的 1/25**，且参考核那部分几乎**全在 BIOS 区**（FreeBIOS 的
等待/延时循环）。也就是说：参考核的 ARM7 真的在 BIOS 里「空转等待」，而本地把这类等待
**算得太快/太省**（本地 ARM7 主要在 0x115C↔0x1160 的 `subs/bgt` 两指令循环里，
按 1 单位/条计费）。

这与续109h 的握手失败**完全吻合**：ARM7 一轮 803 单位 vs ARM9 超时窗口 8600 单位，
差的那 10 倍正是「BIOS 等待时间被少算」的量级。

**下一步（已很明确）**：把 ARM7 的 BIOS 等待/延时循环按参考核口径计费 ——
① 先在参考核侧加一个「SWI 调用 + r0 参数」日志（沿用 `melonds-armstat.patch` 的思路），
