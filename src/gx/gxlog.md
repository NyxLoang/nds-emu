...
### 21-B9yi（续35）— 视体裁剪 + 矩阵栈 + 两处矩阵数学 bug：3D 场景终于「看得懂」了

**现象**：续34 之后 3D 图层铺满整屏，但形状是完全错误的巨大色带/楔形。

**定位手法**（本轮新增，值得保留）：
* `gxclip`：视体裁剪各平面的拒绝计数；
* `NDS_POLYDBG` / 参考核 `REF_POLYDBG`：同帧打印前几个顶点的**裁剪空间坐标 + 矩阵**；
* `NDS_MAT_FRAME` / 参考核 `REF_MAT_FRAME`：帧末打印 `proj/pos/tex` 关键元素；
* `NDS_GXDBG_FRAME` / 参考核 `REF_GXDBG_FRAME`：只 dump 某一帧的命令流；
* `gxexec`：**执行**（而非入队）命令直方图（用来发现「一直在收参数、永远不执行」的命令）。

**修的四件事**：

1. **视体裁剪没做**（`gx_raster_vert_tri`）。按 melonDS `ClipPolygon` 实现
   六平面 Sutherland–Hodgman（`±Position[comp] <= W`，跨越平面时对裁剪空间坐标与
   纹理坐标做线性插值），再扇形三角化。不做裁剪时，跨相机的多边形被
   「透视除 + 钳位」拉成铺满屏幕的楔形——正是续34 看到的色带。
2. **矩阵栈语义与真机不同**。melonDS：投影/纹理矩阵各是**单槽**栈（指针 0/1），
   位置矩阵 32 槽（指针 0..63，取 `&31`）；`MTX_POP` 的参数是**6 位带符号偏移**、
   从指针里**减掉**。本地此前用一个 32 槽共用栈 + 「只接受正偏移」，投影矩阵因此与
   参考核完全不同。
3. **`MTX_SCALE`（0x1B）把第 4 行也缩放了**。melonDS 的 `MatrixScale` **只缩放前 3 行**
   （索引 0..11），第 4 行含透视用的 W 分量、必须保持原样。本地连第 4 行一起乘，
   而命令只给 3 个参数 ⇒ 第 4 行被乘上残留垃圾值。
   **同帧对照证据**：参考核 `pos[15]=4096 / proj[15]=81171`，本地**全是 0** ⇒
   投影 W 报废、顶点算出视体外。
4. **`VTX_10`/`VTX_DIFF`（0x24/0x28）的 10 位坐标被放大 4 倍**。melonDS 是
   符号扩展后**原值**使用（`(s16)((p & 0x3FF) << 6) >> 6`），本地写成 `<< 2`。
   本游戏 `VTX_DIFF` 用了 639 万条，累计误差会把顶点整体推出视体。

**怎么验证**（f=4200 同输入脚本，修正前 → 修正后）：
...
画面                 f=4000 顶屏 (87,145,168) 100% 非黑；
                     build\sc__04000.bmp 已能看出迷宫的石墙/台阶/地砖贴图
无回归               f=1000/2000/3000 画面统计与续34 完全相同
单测                 build\test_nds.exe 931 项 0 失败
```

**结果**：✅ 3D 场景从「无法辨认的色带」变成**可辨认的带纹理迷宫**（与参考核 f=4000
同一地点）。这是「画面对得上」的第一步。

**已知遗留（下一步）**：
* **投影矩阵仍与参考核差一项**：参考核 `proj[15]=81171`、本地仍是 `0`；
  `proj[0]/[5]` 也大 ~14%。表现为取景/视角不同（同一场景但相机不一样）。
  下一步按 `REF_MAT_FRAME` 逐帧追踪「投影是在哪一步被写成这样」。
* 光照/雾/边缘标记未实现：本地直接用纹理色，参考核有 DIF_AMB/SPE_EMI 光照后的色调。
* 颜色口径：melonDS 的 3D 输出是 18 位色（15 位色 ×2+1），本地仍是 RGB555，
  逐像素对照时要记着这个换算。

### 21-B9yi（续36）— `MTX_MULT_4x3` / `MTX_MULT_3x3` 参数布局修正（投影矩阵终于成形）

**现象**：续35 之后场景已能辨认，但取景与参考核不同；同帧对照显示
参考核 `proj=(10708,16060,86432,2227)`、本地 `proj=(12093,18477,0,2344)`。

**定位**：给两边都加了「投影矩阵变化追踪」（`NDS_PROJDBG` / 参考核 `REF_PROJDBG_FRAME`，
打印**是哪条命令改了投影**）与「多参数命令完整参数」dump（`ourpar` / `refpar`）。
两边序列完全一致：
...
用手算验证：参考核 `m[0] = (0x09C2 × 17559) >> 12 = 10708` ✔ 说明 melonDS 与本地
的**公式一致**，差在**参数**：本地取到的是 `0x0B05`，参考核是 `0x09C2`。

**根因**：`MTX_MULT_4x3`（0x19）与 `MTX_MULT_3x3`（0x1A）的参数在 melonDS 里是
**扁平布局**（`s[i*3+k]`，每行 3 个）；本地先调用 `gx_mat_load4x3()` 把 12 个参数
**展开成 4 列**再相乘 ⇒ 从第 1 行起就错位（第 3 行更整行错），投影矩阵的 W 行因此
拿不到正确项。改成直接按扁平布局索引 `p[i*3+k]`（`LOAD_4x3` 仍然是「3 参数/行 + 0」
的 4x4 装载，两者不要混）。

**怎么验证**：同帧对照 `cmd=19` 前后，本地 `proj[15]` 从恒 0 变为非 0；
`build\test_nds.exe` 931 项 0 失败；f≤3000 画面统计与上一轮逐值相同（无回归）。

**结果**：✅ 投影矩阵成形，3D 画面从「色带」→「有纹理的迷宫」→ 现在能看到
**石柱、地砖、栏杆等具体结构**（`build\m4__04000.bmp`），`gxpath` 统计为
`tex=469,462 / drawn=393,368 / px=43,270,642 / vtx0w=0`。

**已知遗留（下一步）**：
1. 游戏自己算出来的矩阵参数与参考核仍不同（同为 `cmd=19`，本地 `0x0B05`、
   参考核 `0x09C2`，同量级但不同值）⇒ 相机取景不同。这是**游戏侧状态**差异
   （相机动画/计时相关），不再是渲染链路问题；下一步沿「相机参数的来源」追
   （TIMER/RTC/`Div` 结果等）。
2. 底屏在这些帧是全黑（参考核有内容）⇒ 底屏内容多半来自**显示捕获/2D 图层**，
   需要和 3D 输出一起查。
3. 光照（`DIF_AMB`/`SPE_EMI`）与 18 位色口径未实现，颜色与参考核有系统偏差。

### 21-B9yi（续38）— 性能：扫描线跨度光栅化（-72.6% 逐像素测试）+ 瓶颈定位

**背景**：续37 之后画面已经对了，但帧率只有 ~32-41 fps（DS 是 60 fps），
需要往「流畅运行」推。

**改动**：把三角形光栅化从「包围盒逐像素 + 三次边函数」改成**扫描线跨度**——
三角形是凸的，本行有效 x 只在三条边与 y 的交点之间；先求跨度（两侧各放宽 1
像素抵消整数截断），**仍在跨度内调用原边函数做精确判定**，因此填充规则与输出
完全不变（实测 `px`/`drawn`/画面统计逐值相同）。

**确定性指标**（与机器负载无关，f=4200 带按键运行）：

```
包围盒像素（旧实现要遍历的量） = 404,267,279
实际测试像素（新实现）         = 110,820,594   → 减少 72.6%
写出像素（两边相同）           =  43,270,642
```

**瓶颈定位**：加了 `NDS_NORAST=1`（只裁剪/三角化、不逐像素光栅化）做对照。
按每次迭代 ~15-20 周期估算，光栅化占整机时间只有 ~1.5% 量级
（110M 次迭代 ≈ 2G 周期 ≈ 0.7 秒，而 4200 帧跑 ~100-130 秒）——**真正的开销在
CPU 解释器与每指令的 IO 推进**（`cpu_step` 每步都要走 `bios_irq_tail9`、
`io_advance_timers`（4 个定时器）、`io_advance_cart`（GX/DMA/卡带时钟））。
→ 下一步性能工作的方向是「无事可做时快速返回」的事件化推进，而不是光栅化。

**怎么验证**：`build\test_nds.exe` 931 项 0 失败；f=1000/2000/3000/4000 两屏统计
与续37 逐值相同（无回归）；`gxwork` 行给出上表的两个计数。
...
## 2026-09-13 · 21-B9yi（续56）：对外新增 `gx_pending()` —— 给 IO 时钟门控用

**做了什么**：新增查询接口 `int gx_pending(const gx_t *g)`，返回「GX 此刻还有没有活」：

```c
return g->busy_cycles > 0 || g_q_len > 0 || g_e_len > 0 || g_need > 0;
```

（忙周期 / 待执行条目 / 未解码完的条目 / 当前命令还缺参数 —— 四者全 0 时
`gx_advance()` 什么也不会做。）

**为什么**：`io_advance_cart()` 每条 ARM9 指令都要调一次，而它的第一件事就是
推进 GX 引擎。本游戏大部分时间 GX 队列是空的，这个查询让上层可以**整段跳过**
这次推进（见 `src/io/iolog.md` 续56）。

**怎么验证**：不单独测（它是门控的一部分），随续56 的门控一起验证 ——
931 项单测 0 失败、1000 帧 disp-change 时间线逐行相同、2000 帧截图 SHA-256 相同、
**12000 帧长跑（满键 73.4 fps）两屏全程有内容、无停摆**（即门控没有漏事件）。

**结果**：✅ 保留。门控整体 A/B：15.56 s → 14.37 s（约 -7.6%）。

## 2026-09-13 · 21-B9yi（续100）：GX 静态状态纳入即时存档

- **背景**：查「读档后不逐字节延续」时逐个排查「不在结构体里的状态」，发现
  `gx.c` 的文件级静态量 —— **GX 命令队列**（`g_q/g_q_head/g_q_len/g_pending`）与
  **解析后的条目流**（`g_ent/g_e_head/g_e_len`）以及参数收集状态（`g_need/g_ccmd`）——
  **从不进即时存档**（它们不是 `gx_t` 的成员）。读档后 GPU 侧会带着「上一个时间线」的
  队列/条目继续跑，与存档时刻不一致。
- **做了什么**：新增三个接口 `gx_state_size()/gx_state_save(void*)/gx_state_load(const void*,size_t)`
  （内容全是定长数组与整数、无指针 ⇒ 直接 memcpy；长度不符则拒绝装载），
  由 `state` 模块作为带长度前缀的块写进存档。
- **验证**：单测 976 项 0 失败；锚点 2000 帧截图仍 `A72E11A2…CC513`（默认路径零回归）。
- **备注**：该块约 50 KB（队列 256×~72B + 条目流 4096×8B），对存档体积影响可忽略。

## 2026-09-13 · 21-B9yi（续108k）：本作 **DISP3DCNT 使用画像** —— 开了纹理/抗锯齿/雾，**没开**描边与 3D 混合

- **怎么测**：长跑时抓 `gx:` 诊断行（DISP3DCNT 变化时打印）。6000 帧（含 3D 场景与雾效）
  里只出现两个值：
  ```text
  gx: fog OFF disp3dcnt=00000000 f=0
  gx: fog ON  disp3dcnt=00000091 f=675
  ```
  `0x91 = bit0(纹理映射) + bit4(抗锯齿) + bit7(雾)`。
- **结论（对实现范围的意义）**：
  * **bit5（edge marking / 描边）本作从不开启** ⇒ 我们没实现描边**对本作无可见影响**
    （melonDS `ScanlineFinalPass` 里那段 edge marking 属「别的游戏才需要」）；
  * **bit3（3D alpha blending）也没开** ⇒ 当前实现与该位语义一致，没有漏掉
    「半透明多边形」这一路（多边形 alpha 仍参与纹理 alpha 与最终 2D 合成）；
* **bit4（抗锯齿）开了、而我们没实现** ⇒ 这是**已知视觉差距**：真机/melonDS 会把 3D
  边缘像素与下层融合（melonDS 用 `AttrBuffer` 覆盖位 + `ScanlineFinalPass`），
  本地目前是硬边。**列为下一步候选功能**（描边不做，AA 要做）。

## 2026-09-13 · 21-B9yi（续108l）：**实现 3D 抗锯齿**（DISP3DCNT bit4，本作开着）

**参考核口径（melonDS）分两步**：

```text
<1> ScanlineFinalPass（3D 侧）：按每像素覆盖度把「顶层 3D 像素」与「被压下去的 3D 像素」
    混合——颜色只在下面那层不透明时混，alpha 一律混：A = (A₁·cov + A₂·(32-cov))>>5；
<2> 2D 合成阶段：再用这个 alpha 把 3D 图层与 2D 图层混合（于是 3D 轮廓会与背景抗锯齿）。
```

**本地实现（完全对应这两步）**：

* `gx_t` 新增三组平面：`cov[]`（每像素覆盖度 0..31）、`fb2[]/fba2[]`（被压下去的前一层）。
* 光栅化的**两条路径**（带纹理那条、以及平色 `gx_raster_tri`）都做：
  - 抗锯齿开启且片元不透明（alpha=31）时，用 **4 子样本**（像素放大 2 倍，子样本取
    ±1,±1，即四角）算覆盖度：0/7/15/23/31；覆盖度 0 的片元不写（露出下层）；
  - 写新像素前把旧的**不透明**像素下推到 `fb2/fba2`（melonDS `ColorBuffer[+BufferSize]`）。
* 合成阶段（`render.c` 的 `render_3d`）：覆盖面 <31 时先做 `<1>`（颜色+alpha），
  再按 alpha 与「下层已合成的颜色」（简单路径=直写帧缓冲；否则=`px[i].top`）混合。
* 开关：`NDS_NOAA=1` 关闭抗锯齿（A/B 与诊断用）。

**验证**：

```text
单测 982 → 989 项 0 失败（新增 7 项）：覆盖度默认 31 / 内部像素 31 / 斜边存在 0<cov<31 /
  「再画一遍不透明三角形」时旧像素确实被下推到 fb2/fba2（颜色+alpha）
锚点 2000 帧：**两屏统计与改动前逐值相同**（顶 24,36,40 / 底 52,81,108 —— 线性混合保均值），
  截图 SHA-256 A72E11A2…CC513 → **74D73A3D6552E993…58314**（就是抗锯齿带来的边缘变化）
像素级对照（同帧、同输入）：顶屏 8,288/49,152 个像素变化，其中**没有一个变成纯黑**
  （>24 的大变化只有 160 个，其余是 ≤8 的柔和边缘混合）
性能（相邻 A/B，600 帧窗口同输入）：关 AA 10,131 ms / 开 AA 10,127 ms（59.2 fps），
  ppu 渲染 725 ms → 762 ms ⇒ **约 0.06 ms/帧**，可忽略
即时存档往返（tools/statetest.ps1）仍 **PASS**（新平面在 `gx_t` 里，随 IO 一起入档）
```

**已知限制**：覆盖度用「四角子样本」近似（melonDS 用扫描线跨度精确算），
所以边缘像素的覆盖度是 4 档（7/15/23/31）而不是全 32 档；对「边缘更柔和」这个效果足够，
若将来要与参考核逐像素对账可以再细化。
