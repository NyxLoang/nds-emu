...
### 21-B9yi（续20）— `NDS_DMAIRQ` 诊断 + IF bit11 来源（GX-FIFO DMA 无节拍）

新增 `NDS_DMAIRQ=LO-HI`：每次 DMA 完成中断上挂时打印
（帧、核、通道、`cnt_h/cnt_l`、`sad/dad`、发起 PC）。f=1990-2070 本地 58 次 IF bit11
**全部**是：

```
arm9 ch=3 cnt_h=7C40 cnt_l=308 sad=02370D0C dad=04000400 pc=01FF8300
cnt_h=0x7C40 = 使能 + 32 位 + 模式 7（GX FIFO）+ bit14「IRQ on end」
```

而参考核同窗口的 DMA3 写全是 `cnt_h=0x8400`（模式 0 立即、无 IRQ）+ 主存↔VRAM 搬运
⇒ 两侧连 DMA3 用途都不同（游戏状态已分叉），且本地**模式 7 DMA 一次搬完**，
立刻 `RemCount==0` ⇒ 立刻挂中断（melonDS 受 FIFO 112 项容量与 3D 消耗速度限制，
`Run9` 会 `Stall` 分批推进）。
...
### 21-B9yi（续44）— IO「首次访问」判定改 O(1) 位图 + 长跑/输入行为验证

**改动**：`io_addr_first_seen()` 原来是「256 项数组 + 线性扫描」，而该函数在**每次
IO 访存**都要调用（诊断用途：未实现寄存器只提示一次）。改成按地址偏移的 8KB 位图
（`0x04000000-0x04001FFF`），O(1)。

**A/B 实测（同机交替，各 2 次 4200 帧）**：

```
线性扫描基线：132.7s (31.6fps) / 132.5s (31.7fps)
位图 O(1)   ：133.2s (31.5fps) / 133.3s (31.5fps)   → 差异在噪声内（≈0）
```
⇒ 说明 IO 轮询频率低于预期（已见地址表也不长），此项**不是**性能瓶颈。
保留该改动（算法更优、无行为变化），并记入「零收益实验」清单。

**长跑与输入行为（本轮新增证据）**：

```
20000 帧（mask 0x3F7）：两屏全程有内容、无停摆；f≈6000 后画面基本静止
                        —— 游戏在**等输入**（该掩码恰好不含 START）
9000 帧（mask 0x3FF，含 START）：f=8000/9000 换成新画面（顶屏地牢 + 底屏
                       对话框/文本），说明**按键被游戏正确接收、剧情在推进**
```

## 2026-09-13 · 21-B9yi（续55/56）：IO 推进的两道门控 —— 相邻配对 A/B **-15.5%**

**背景**：`NDS_PROF` 细粒度剖析（续55 把 IO 推进再拆成「定时器」与「卡带/GX 时钟」
两块）显示这两块在每步开销里都占约 18%（含插桩自身的 rdtsc 开销），
是解释器核心之外最大的可动项。两者都不需要在「没事干」时逐指令全量推进。

### 续55：定时器「使能位图」

* `io_t` 新增 `uint8_t timer_on[2]`（bit i = TMi 使能），维护点是 `io_write8` 写
  定时器寄存器之后**重算**（写控制寄存器很罕见）。
* `io_advance_timers()` 改成**只遍历开着的定时器**（`__builtin_ctz` 取最低置位），
  一个都没开时就是「读一次 + 判断 + 返回」。
  背景：本游戏同一时刻通常只开 1-2 个定时器，旧实现每条指令固定 4 次调用。

**A/B（3 轮交替、各 1000 帧、满键 0x3FF）**：

```
续55（位图）   ：15.60 / 15.49 / 15.49 s   → 平均 15.53 s
续54（改动前） ：17.00 / 17.00 / 16.99 s   → 平均 17.00 s     ⇒ 约 -8.6%（帧率 +9.5%）
```

### 续56：「卡带/GX 时钟」整体门控

`io_advance_cart()` 内部是「GX 引擎推进 + GX-FIFO DMA 续跑 + 卡带取数循环 +
完成中断」，本游戏大部分时间这几件事都不忙。

* `io_t` 新增 `uint8_t cart_clock_on`，语义 = **还有活**：
  GX 有待执行/待收参数/忙周期（新增 `gx_pending()`）∥ 卡带在等取数或预取 FIFO 有货或
  DRQ 置位（`io_cartbus_busy()`）∥ GX 模式 DMA 通道 `rem != 0`（`io_dma_gx_busy()`）∥
  卡带完成中断待挂。
* **置 1 的时机**（任何可能产生新活的路径）：GX 命令写 `io_gx_write32`、
  卡带寄存器写、DMA 寄存器写、卡带数据端口读/写。
* **重算时机**：`io_advance_cart()` 末尾按各模块真实状态重算，全闲则清零。
* 调用点（`cpu_step`）先判一次 `cart_clock_on`，为 0 时**整段跳过**。

**A/B（3 轮交替、各 1000 帧、满键 0x3FF）**：

```
续56（门控）   ：14.62 / 14.28 / 14.20 s   → 平均 14.37 s
续55（改动前） ：15.61 / 15.52 / 15.56 s   → 平均 15.56 s     ⇒ 约 -7.6%（帧率 +8.2%）
```
...
```
931 项单测 0 失败
1000 帧 disp-change 时间线（70 行）逐行相同
2000 帧双屏统计逐值相同；最终截图 SHA-256 = A72E11A2…CC513（与前几轮完全一致）
12000 帧长跑（满键）：163.4 s（73.4 fps），两屏全程有内容、无停摆
                     —— 这是门控最需要证伪的点（漏事件会让卡带/GX 停摆）
窗口 3000 帧（满键）：70.7 s (42.4 fps) → 57.4 s (52.3 fps)
窗口 600  帧（满键）：5.30 s (113 fps)
```

**结果**：✅ 两道门控都保留。本轮合计 **-15.5%**（帧率 +18%）。

## 2026-09-13 · 21-B9yi（续59）：**触摸被游戏接收**的关键一位 —— EXTKEYIN bit6（PENIRQ）

**背景**：续58 查到「游戏在运行期从不读触摸数据寄存器（AUXSPIDATA）」，当时只能把
触摸游戏侧验证列为开放项。本轮用**参考核**把它查穿了。

**参考核实验（melonDS + 临时补丁，见 `tools/ref/README.md`）**：

```
12500 帧、满键、f=11800 起每 40 帧注入一次触点（ADC 0x7F0/0x5F0）：
  参考核 不触摸：refspi: datareads=842          （与本地一致）
  参考核 注入触摸：refspi: datareads=18122      （≈24 次/帧！）
⇒ 游戏确实会读触摸，而且是「有触摸才读」
```

**根因**：看 melonDS `TSC::SetTouchCoords()`：

```cpp
if (y == 0xFFF) { NDS.KeyInput |=  (1 << (16+6)); return; }  // 抬起 = 置 1
TouchX <<= 4; TouchY <<= 4;
NDS.KeyInput   &= ~(1 << (16+6));                            // 按下 = 清 0
```

也就是说 melonDS 把触摸笔状态建模成 **KeyInput bit22 = EXTKEYIN(0x04000136) bit6
= PENIRQ（低有效）**。游戏先读这一位判断「有没有笔按下」，**有才去轮询 SPI 拿坐标**。

而本地 `io_read8()` 里那一行是：

```c
case 0x04000136u: return 0x7Fu;   /* X/Y 等高位按键：1=松开 —— bit6 永远“抬起” */
```

⇒ 游戏永远认为没有触摸，自然永远不会去读触摸数据寄存器（续58 的悬案）。
...
（`io->touch.down` 由 `io_set_touch()` → `touch_set_pos()` 维护，窗口鼠标触摸与
runner `--touch-*` 走的是同一条路，所以两边同时生效。）

**怎么验证**：

```
3000 帧、f=100 起每 40 帧注入触点，监视 AUXSPI 全部读：
  修复前：读 SPIDATA 0 次（f≥1）
  修复后：读 SPIDATA 70080 次（≈24 次/帧，与参考核 24.7 次/帧同量级）
游戏侧行为：同一输入下「无触摸 vs 注入触摸」的画面统计从 f≤500 起就不同，
  最终截图 SHA-256 不同 ⇒ 游戏**确实在响应触摸**（不再是“零影响”）
零回归：不触摸时与修复前**逐字节一致**（2000 帧统计逐值相同、
  截图 SHA-256 仍为 A72E11A2…CC513）；931 项单测 0 失败
```

**结果**：✅ 保留。这一步把「触摸」从**开放项**变成**闭环能力**：
窗口里用鼠标点底屏，游戏现在真的能收到。

## 2026-09-13 · 21-B9yi（续72）：触摸链路的两个「已查清」与一个「仍然开放」

### 已查清 ①：早期画面本来就是「碰哪里都行」

在已知会响应触摸的早期画面（3000 帧、满键脚本）比较两个相差很远的触摸位置：

```
本地  ：位置 (128,96) 与 (240,180) → 统计与截图哈希**完全相同**
参考核：位置 A（ADC 0x800,0x600）与 B（ADC 0xF00,0xB40）→ 统计**完全相同**
```

⇒ 「本地不同位置无差异」不是我们的坐标解码问题，而是**该画面本身对位置不敏感**
（触摸只当“有没有按”用）。这条排除了一个可能的嫌疑。

### 已查清 ②：与参考核的「注入刻度/校准口径」不同（**已记录，未改**）

```
melonDS：TSC 回复 = ADC<<4；默认校准 ADC 0↔像素 0、ADC 0xFF0/0xBF0↔像素 255/191
本地   ：TSC 回复 = ADC（12 位原值）；固件校准 ADC 0x200↔像素 33、ADC 0xE00↔像素 225
```

两者**各自自洽**（注入端换算与游戏端解析用的是同一套常量，游戏算出的像素都是对的），
所以本轮**不做推测性改动**——真要改成参考核口径，必须同时改
「固件校准常量 + 注入端换算 + 单测」，且需要一次能观测「位置敏感」的界面来验证。

### 仍然开放：战斗内「拖拽下令」

参考核在同一条脚本下、f=18000 的画面**因触摸而不同**（`181,190,205` → `154,156,174`），
而本地同一帧两组**完全相同**（只差 1~11 个 ARM7 中断）。但两边同一帧的画面内容截然不同
（本地 `26,41,38` 暗场景 vs 参考核亮画面）⇒ 处于**不同游戏状态**，
本地那一相位可能本就不是「等待玩家指令」的相位。**该开放项需人工试玩确认**，
自动化脚本目前到不了那个相位。

### 已查清 ③（本轮正面结论）：**坐标数据端到端有效、且随位置变化**

直接观察「游戏从 AUXSPIDATA 读走的字节」在两个不同触摸位置下的差异
（`--watch-r 040001C2-040001C4`，触摸在 f=200 注入）：

```
位置 A 像素(128,96) → ADC X=0x7F0 → 回复字节 (第2,第1) = (0x80, 0x3F)
位置 B 像素(240,180) → ADC X=0xEF0 → 回复字节 (第2,第1) = (0x80, 0x77)
```

按 `touch_reply()` 的 12 位口径手算：X=0x7F0 → `b1=(0x7F0>>5)&0x7F=0x3F`、`b2=(0x10)<<3=0x80`；
X=0xEF0 → `b1=(0xEF0>>5)&0x7F=0x77`、`b2=0x80` —— **与实测读值逐个吻合**。

⇒ 我方链路「像素 → ADC → TSC 回复字节 → 被游戏读走」**全程正确且带位置信息**。
结合「早期画面本来就该对位置不敏感」（参考核同样如此），可以确定：
**游戏在我们能到达的相位里不按位置响应**，而不是我们的坐标数据不对。

## 2026-09-13 · 21-B9yi（续73）：TSC 通道语义**按参考核对齐**（Z1/Z2/BAT/TEMP 与 8 位模式）

**发现**：melonDS 的 `TSC::Write()` 只处理三种通道 ——
`0x10`→TouchY、`0x50`→TouchX、`0x60`→麦克风；**其余通道一律 `ConvResult = 0xFFF`**，
并且 8 位模式只是 `ConvResult &= 0x0FF0`。而本地此前自己编了一套：
`Z1=0x300/0xFFF`、`Z2=0xB00/0x000`、`BAT=0x000`、`TEMP=0x000`，8 位模式另写了拆分逻辑。

这条差异值得追的原因：实测里**游戏 ARM7 的触摸驱动轮询的正是通道 0（TEMP0，命令字节 0x84）**
——我们返回 0、参考核返回 0xFFF。若游戏在更严格的相位（如战斗）用它做“是否有效触摸”的判定，
就会导致“触摸送不到”。**这个假设最终被实验否掉**（见下），但口径本身确实该对齐。

**改动**（`src/io/touch.c`）：

```c
case TSC_CH_X:   return t->down ? t->adc_x : 0x000u;   /* 不变 */
case TSC_CH_Y:   return t->down ? t->adc_y : 0xFFFu;   /* 不变 */
case TSC_CH_AUX: return 0x800u;   /* 麦克风静音：参考核 sample^0x8000 后 >>4 */
default:         return 0xFFFu;   /* Z1/Z2/BAT/TEMP：与参考核一致（此前是自编值） */
```

8 位模式改为「先把结果掩到 0xFF0，再走 12 位拆分」（此前是另一套拆分）。

**验证**：

```
931 项单测 0 失败（有意思的是 17.2 用例里 8 位模式的期望值 0x15/0x00 正好就是参考核口径，
  说明当年写用例时参考的就是 melonDS —— 反而是实现跑偏了）
早期画面触摸响应仍有效：注入触摸后 f=500 变黑、f=1000 均值改变（与对齐前一致）
**无触摸路径零回归**：3000 帧截图哈希 4F49A16B…（与对齐前一模一样）
战斗窗口对照（f=12000 起每 200 帧点 (90,78)）：仍只有 1 个 ARM7 中断之差
  ⇒ “触摸在战斗相位不被接受”**不是**由这组通道值造成的（假设被否掉）
```

**结果**：✅ 保留（与参考核口径一致的保真度修正，且无回归）；
「战斗内下令」仍是人工试玩确认的开放项。

## 2026-09-13 · 21-B9yi（续74）：**触摸在玩法层面生效** —— 之前测不出来是脚本不自然

**关键认识**：早期几轮「触摸无影响」的结论都建立在**持续按住全部按键（mask 0x3FF）**
的脚本上；而随机输入浸泡显示这种脚本会让游戏**在少数状态间原地循环**，
随机输入才能把游戏带过更多状态。

**对照实验**（同会话、并行、唯一差别是触摸）：

```
A：--key-random 12345 --key-period 40                              30000 帧
B：--key-random 12345 --key-period 40 --touch-random 999 --touch-period 100
结果：12 个采样点里 7 个不同（f=5000/7500/10000/12500/22500/25000/30000）
      最终截图 SHA-256 不同（D0EDDFB8… vs C9C0C1E3…）
      summary 不同（vram-nz 186351 vs 187198；irq9 7227805 vs 7199359）
```

⇒ **触摸确实在改变游戏进程**。触摸这条线至此三层闭环：
寄存器层（ARM7 按 ~24 次/帧轮询 SPI）、数据层（位置不同→读到的字节不同且与手算吻合）、
**玩法层（自然输入下加/不加触摸走向不同状态）**。

## 2026-09-13 · 21-B9yi（续87）：**未知 IO 逐地址核账** —— 140 → 126，且「参考核有、我们漏」的地址归零

- **动机**：此前只知道「未知 IO 稳定 139/140 种」，但**从没逐个看过它们是什么**。
  未知寄存器 = 潜在的玩法 bug，必须拿参考核的寄存器表核一遍。
- **做了什么**（先把账本做出来，再补账）：
  1. `io_mark_seen()` 现在按**读/写**分别记位（bit0=读过、bit1=写过），
     并新增 `io_unknown_report()`：跑完一次性列出**全部**未知地址与读写方向
     （`NDS_UNKIOSUM=1` 打开，输出形如 `unknownio: n=140 400004C-w 40000E8rw …`）；
  2. 按参考核（melonDS `NDS.cpp` / `SPU.cpp`）逐个核账，补上**三处它实现了而我们没有**的：
     * **MOSAIC**（引擎 A `0x0400004C` / 引擎 B `0x0400104C`，16 位，两个引擎各一份）；
       实测本游戏只在开机写 `0x0000`（禁用）⇒ 只做寄存器语义，不实现马赛克渲染；
     * **DMA9Fill**（`0x040000E0-0x040000EF`，4×u32）：参考核当普通可读写寄存器，
       本游戏在 `0xE8-0xEB` **读写**它——「写进去读回来」正是我们此前做不到的（永远返回 0）；
     * **声音捕获寄存器**（`0x04000508/509` Cnt、`0x04000510/518` DstAddr、`0x04000514/51C` Length）：
       参考核口径是 Cnt/DstAddr 可读回、Length 只写；本地此前整段未实现（读回 0）。
- **核账结果（20000 帧自然输入，`--key-random 12345 --key-period 40 --touch-random 999`）**：
  ```
  改动前：unknownio: n=140
  改动后：unknownio: n=126   （消失的 14 个 = MOSAIC 4 + DMA9Fill 4 + SNDCAP 2 + 引擎B MOSAIC 4）
  剩余 126 个逐类核对参考核：
    0x04000056-5F / 0x04001056-106B   ← 参考核只实现到 BLDY(0x54)/MASTER_BRIGHT(0x106C)，这段同样不实现
    0x04000330-0x040003BF             ← 参考核 NDS.cpp 里**一个 case 都没有**（完全未实现区）
    0x04001006/07                     ← 引擎 B 的 VCOUNT：参考核也没有这个 case
  ⇒ 「参考核实现了、我们漏掉」的地址 = **0 个**
  ```
- **零回归（两条独立证据）**：
  ```
  锚点脚本 2000 帧：f=2000 顶屏 24,36,40 / 底屏 52,81,108（同前）
                    截图 SHA-256 = A72E11A2…CC513（与前完全相同）
  20000 帧自然输入：vram-nz=393216 / irq9=4691855 / irq7=262361（与续77 记录逐值相同）
                    savechip hash=3A361368EF684AD7（未存档，同前）
  单测：931 → **947 项**（新增 16 项：MOSAIC 字节/半字与两引擎独立、DMA9Fill 字节/半字/整字读回、
        SNDCAP Cnt/DstAddr 读回 + Length 只写 + 声音寄存器对 ARM9 不可见）
  ```
- **顺带查明的两个细节**：①MOSAIC 是 16 位寄存器，`0x4E/0x4F` 属于「多余字节」，
  参考核同样不实现（已按忽略处理，不再报未知）；②声音寄存器只在 **ARM7** 侧可见，
  从 ARM9 读回 0（单测把这条也钉住了）。
- **结果**：✅ 保留。

## 2026-09-13 · 21-B9yi（续108）：固件用户设置的**校准口径开关** `NDS_FW_CAL=flat`

- **背景**：跨核对账时发现，主存 0x027FFC80 起 0x70 字节（**固件用户设置**，
  游戏在启动时从 SPI 固件读出、原样拷进这里）在两边不一样：
  ```text
  本地默认： 58: 00 02 00 02 21 21 00 0E 00 08 E1 81   ← ADC 0x200↔px33、0xE00↔px225
  参考核：   58: 00 00 00 00 00 00 F0 0F F0 0B FF BF   ← ADC 0↔0、0x0FF0↔255
  ```
  两边**各自自洽**（本机窗口鼠标→ADC 的换算 `runner_touch_adc()`/main.c 的鼠标触点
  与本地校准配套；参考核与它自己的触控管线配套），但游戏会按这份设置决定触控行为
  ⇒ 跨核对账时它是一处**已知混杂项**（实测游戏会因此多发一条 IPC 消息、f≈285 起分叉）。
- **做了什么**：`src/io/touch.c` 的 `fw_user_body()` 增加环境开关 `NDS_FW_CAL=flat`，
  打开后用户区的触屏校准换成参考核那一套（x1/y1=0/0、x2=0x0FF0→255、y2=0x0BF0→191），
  便于做「单变量」对照；**默认关闭**，手感与既有验收口径不变。
- **验证**：`NDS_FW_CAL=flat` 下 0x027FFC80+0x58 的字节与参考核一致；
  单测 982 项 0 失败（该开关不影响默认路径）；实测**那条多出来的 IPC 消息仍在**
  ⇒ 说明它不是校准引起的，而是**消息刷新时机**的问题（见
  [`docs/21-rom-bringup.md`](../../docs/21-rom-bringup.md) 续108 的「下一步」）。
