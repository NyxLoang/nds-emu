...
## 2026-09-13 · 21-B9yi（续93b）：把「欠账上限」改成时间制（100 ms）+ 重帧逐帧诊断

- **接着续93 的问题**（重场景 58.4 fps，目标 59.83）继续定位：
  - **重帧逐帧诊断**（`--fps-every` 打开时生效）：单帧 >25 ms 就打一行
    `heavy: f=… tot=… evt=… emu=… render=…`。9000 帧里只有 **4 个**这样的帧
    （f=1 启动、f=742、f=2953、f=8456，25~27 ms，**模拟占 20~24 ms**）——
    它们本身解释不了 1.3 s 的损失。
  - **改用 `pace:` 的「丢帧重同步」计数**看真凶：重场景每 3000 帧有 **27~37 次**
    重同步。原来「落后超过**一帧**就重置截止时刻」会**把整段欠账丢掉**，
    而重场景里恰恰是「一串略超预算的帧」把欠账累到一帧以上 ⇒ 每次丢几十毫秒，
    合计正好是那 1.3 s。
  - **改成时间制上限**：欠账 ≤ **100 ms** 就保留（下几帧少等、把突发补回来），
    超过 100 ms 才重新对齐。为什么不无限保留：**声卡是按墙钟推进的**，
    游戏时间必须跟住墙钟，否则长时间漂移会越来越大 ⇒ 用 100 ms 吸收突发、又让漂移有界。
- **效果**（12000 帧、同脚本、1×）：
  ```
  改前：57.6 / 59.8 / 58.7 / 59.8（另一轮 58.3/58.3/59.8/59.6）
  改后：58.3 / 58.3 / 59.8 / 59.6 → 平均 **59.0 fps**（原来 58.4）
        重同步次数从 27~37/3000 帧降到 0~11/3000 帧
  ```
  ⇒ 轻/中场景现在能**精确**跑到 59.8（目标 59.83）；重场景仍有 ~1.4% 的损失，
  来自「少数帧工作量确实超过一帧预算」，只能靠**性能优化**再压。
- **负结果（诚实记录）**：试过把睡眠阈值从 1.5 ms 提到 3 ms（少睡多转、多烧 CPU），
  fps **完全没变**（58.3/59.8/58.7 一模一样）⇒ 误差不来自 `SDL_Delay` 粒度，已还原。
- **结果**：✅ 保留（时间制欠账上限 + 两条诊断）。

## 2026-09-13 · 21-B9yi（续95）：快进时**隔帧渲染**（把宿主开销省掉）

- **动机**：续92b 实测快进在重场景只有 ~1.1×；而快进时音频已静音（续92），
  此时画面只是「看着在动」，没必要每帧都做宿主渲染（`ppu_render` + 清屏/菜单/Present
  实测 2.4~4.8 ms/帧）。
- **做了什么**：`ff_hold`（按住 Tab / `NDS_FF_HOLD=1`）时**隔帧跳过宿主渲染**
  （`frames_done & 1`），模拟与音频状态照常推进。正常游玩（1×）不受影响。
- **实测**（`NDS_FF_HOLD=1 --frames 3000`，含轻/重场景）：
  ```
  分段：97.4 / 59.4 / 60.9 fps     平均 69.0 fps（改前 67.1）
  60 帧 1× 冒烟：9.99 s（59.7 fps）——正常路径无变化
  ```
  ⇒ 轻场景收益明显（首段 97 fps），**重场景仍被模拟速度卡住（~60 fps）**，
  所以整段平均只快约 3%。结论与续92b/94 一致：快进的天花板是**模拟速度**，不是渲染。
- **结果**：✅ 保留（快进专用；正常游玩逐帧渲染不变）。

## 2026-09-13 · 21-B9yi（续102）：Ctrl+C 也能优雅退出（写回存档）

- **动机**：窗口模式的退出路径会写回 `.sav`（以及 `--save-state` 的即时存档），
  但从终端里按 **Ctrl+C** 结束进程时，默认行为是**直接终止**，存档就丢了。
- **做了什么**：加 `volatile sig_atomic_t g_stop_requested` + 信号处理函数，`main` 里注册
  `SIGINT`（Windows 另加 `SIGBREAK`；POSIX 另加 `SIGTERM`），主循环每轮检查该标志：
  一旦置位就打印「收到停止请求，准备退出（写回存档）…」并走**完整退出路径**
  （写 `.sav` / `--save-state` / 关闭音频 / 释放资源）。
- **验证状态（诚实记录）**：代码路径已编译通过、单测 976 项 0 失败；
  但**本机自动化无法向控制台进程投递真正的 Ctrl+C 事件**（试过向 PTY 写 ^C，
  进程不受影响）⇒ 这条留作**人工验证**：运行窗口后按 Ctrl+C，应看到
  「收到停止请求」并打印 `save : stored …`。
- **顺带观察**：本次窗口运行（无按键脚本、某个重场景）实测 **~52 fps**（模拟 18 ms/帧），
  说明个别场景仍低于 59.83 的目标 ⇒ 与续93b/99 的结论一致：**瓶颈是模拟速度**。

## 2026-09-13 · 21-B9yi（续103）：**音画漂移**量化并修掉（欠账上限 100 ms → 30 s，允许追帧）

- **问题**：声卡是**按墙钟**推进的（SDL 回调），而重场景模拟只能跑 ~52 fps ⇒
  游戏时间比墙钟越落越远。给 `--fps-every` 加了一行 **`drift:`** 读数（本段模拟时间 vs 墙钟），
  实测（12000 帧、1×、带按键脚本）：
  ```
  默认（欠账上限 100 ms）：分段 +4645 / +1069 / +1900 ms ⇒ 整段 208.2 s（理想 200.6 s）
                          **累计音画漂移 ≈ 7.6 秒**（声音超前画面 7.6 s）
  ```
- **修法**：把「欠账上限」从 100 ms 放宽到 **30 s** ⇒ 重场景掉队后，随后轻场景会**追帧**补回来
  （掉队期间不等待、跑满当前最快速度）。
  ```
  新默认（30 s 上限）：分段 +604 / -506 / -105 ms ⇒ 整段 **200.58 s**（理想 200.6 s）
                       平均帧率仍 **59.8 fps**，**漂移 ≈ 0.1 秒**
  ```
- **代价（诚实记录）**：追赶期间会短暂跑得比真机快（分段实测 60.3 fps），视觉上几乎察觉不到；
...
## 2026-09-13 · 21-B9yi（续105）：**8.4 分钟窗口长跑 —— 音画漂移 ±1 ms 内**（续103 效果的长期证据）

- **怎么测**：窗口模式、带音频、1× 帧节奏、带键盘脚本（`--key-frame 1 --key-mask 0x3FF --key-period 120`），
  跑 **30000 帧**（约 8.4 分钟），每 6000 帧打印一次分段帧率与 `drift`。
- **结果**：
  ```
  每段都是 59.8 fps（目标 59.83）；drift 分别为 -6.7 / -0.7 / +0.3 / -0.7 / +1.3 ms
  合计 30000 帧 501454 ms（理想 501450 ms）⇒ 与墙钟**几乎完全一致**
  savechip 与短跑一致（24 字节 / 同一哈希）、全程无停摆
  ```
  ⇒ **长时间游玩音画不漂移**（续103 把欠账上限放宽到 30 s、允许追帧的直接收益；
  对比：默认还是 100 ms 时，12000 帧就累计漂移 7.6 秒）。
- **顺带确认**：这一轮同时验证了「帧节奏 + 音频回调 + 追帧」三者长期共存不会互相打架
  （音频回调按墙钟走、游戏时间跟住墙钟 ⇒ 二者不越拉越远）。

## 2026-09-13 · 21-B9yi（续107）：修掉 `--dump` 前缀的 **use-after-free**（Windows）

- **症状**：同一命令行两次运行结果不同；`--dump build\xx` 有时出现 `dump : _mainram.bin`
  （**前缀整段丢掉**），于是 4MB 镜像被写进**当前目录**——仓库历史上那批根目录 `_*.bin`、
  以及本轮「读档一致性」实验里几次自相矛盾的结论，都是这个 bug 造成的。
- **根因**：Windows 路径解析参数用的是 `CommandLineToArgvW()` 的 `wargv`，而 `--dump`
  当时保存的是**指针** `dump_prefix_w = wargv[i+1]`：
  ```c
  dump_prefix_w = wargv[i + 1];      /* 解析阶段 */
  ...
  LocalFree(wargv);                  /* ROM/存档装载之后就把 argv 释放了 */
  ...
  if (dump_prefix_w != NULL)
      dump_state(nds, dump_prefix_w); /* ← 跑完之后才用：已释放内存 */
  ```
  释放后的内存会被后续分配复用 ⇒ 前缀内容不确定（常表现为空串）。
  `--shot` 之所以一直正常，是因为它当时就拷进了静态 `shot_buf`；`--load-state/--save-state`
  也各自拷进了静态缓冲。**只有 `--dump` 漏了。**
- **修法**：解析时即拷进 `static wchar_t dump_prefix_buf[512]`（与 `--shot` 同口径），
  之后无论 `wargv` 何时释放都安全。
- **顺带**：新增验收工具 `tools/statetest.ps1`（「存档 → 读档 → 继续跑」与「连续跑」逐字节对账，
  9 个内存镜像 + 失败时打印被比较命令的尾部输出），把这类错误的成本压到一次命令；
  用法写在 README 与 `tools/ref/README.md`。
- **验证**：`tools/statetest.ps1` 5 组用例全 PASS（固定脚本 300+1000 / 1000+2000、
  随机按键+触摸 400+400 / 1000+1000、拖拽手势 200+200 与 **230+200（手势中存档）**）；
  锚点 2000 帧截图 `A72E11A2…CC513`、`savechip 3A361368EF684AD7`、单测 976 项 0 失败。

## 2026-09-13 · 21-B9yi（续107c）：**无头路径也写回 `.sav`** + 存档装载消息不再永远说 “loaded”

- **问题 1（功能缺口）**：存档写回只写在**窗口**退出路径上，无头路径
  （`--headless-frames`）在 dump 之后就 `return`，**从不写回 `<rom>.sav`**。
  后果：无头跑出来的「游戏内存档」一退出就丢，而且「游戏内存档」这项在自动化里
  连文件都看不到，只能靠人工。
- **问题 2（诊断误导）**：启动时的 `save : loaded <path>` 是**无条件**打印的
  —— `save_load_file*()` 对「文件不存在」也返回 0（保持 0xFF 全新态），
  于是「没有存档文件」和「读到了存档」在日志里长得一模一样（本轮的读档排查里
  一度因此误判「存档被写过」）。
- **改法**：
  ```c
  static void save_writeback(nds_t *, const wchar_t *); /* 窗口/无头共用 */
  static int  save_file_exists(const wchar_t *);        /* GetFileAttributesW / fopen 探存在 */
  ```
  无头退出分支补 `save_writeback(nds, save_path)`；窗口路径改用同一个函数；
  装载消息改成 `save : loaded …` / `save : new    …`（两个词都对齐成 6 字符宽，便于对日志）。
- **验证**：
  ```
  无存档时：save : new    tools\rom_ascii.sav (8192 bytes) … 收尾 save : stored tools\rom_ascii.sav
  再跑一次：save : loaded tools\rom_ascii.sav (8192 bytes)   ← 两条消息现在能区分了
  两次锚点 2000 帧：截图 SHA-256 同为 A72E11A2…CC513、savechip 同为 3A361368EF684AD7
  6000 帧（带/不带该 .sav）：末帧截图哈希完全相同（970D6C…02DD）⇒ 这 24 字节不改变游戏进程
  ```
- **顺带确认（开放项的客观边界）**：20000 帧随机按键+触摸跑完，存档芯片仍是
  **nonzero(vs 0xFF)=24**（这 24 字节是开机写的状态寄存器内容，不是存档数据）
  ⇒ **自动化跑不出「游戏内存档」**，「游戏内存档菜单保存」继续是**人工试玩验收项**；
  但从此人工保存出来的 `.sav` 无头模式也会写回，后续可以拿真实存档做自动化回归。
...
## 2026-09-13 · 21-B9yi（续108d）：续108 之后的**新基线**（20000 帧自然输入 + 每帧指令量）

- **稳定性**：20000 帧自然输入（`--key-random 12345 --key-period 40 --touch-random 999
  --touch-period 100`）跑完无停摆：
  ```
  vram-nz=393216  disp=00161F10  dispb=00011035  irq9=4692642  irq7=262371
  savechip: 24 字节 / 3A361368EF684AD7（未存档，同前）
  ```
  （与续77 的同口径记录只差千分之几，属续108 改动后的正常偏移；以后拿这组数当基线。）
- **每帧指令量**（同一跑完的汇总行）：ARM9 `cyc=5511022625`、ARM7 `cyc=715200677`
  ÷ 20000 帧 ≈ **275k 条/帧（ARM9）、36k 条/帧（ARM7）**。
  这条数与「本地 ARM7 的 IPC 消息比参考核多约 19%」放在一起看，指向同一条**保真度**线索：
  本机对每条指令只计 1 个系统单位、**没有建模访存等待周期**，于是相对参考核
  ARM7 跑得偏快、ARM9 与 ARM7 的相对速率也偏了一截（真机/参考核 ARM9 是 ARM7 的 2 倍频）。
  下一轮先做「两边两核每帧指令数」的量化对照，再决定是否引入按区域的访存代价模型。
