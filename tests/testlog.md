...
## 2026-09-06 · 21-B9x — ARM9 IRQ BIOS 六字帧用例
- `[case 21-B9c]` 补：IRQ 触发后 IRQ SP 已下移 0x18，IRQ 栈上按序保存
  r0-r3/r12 与被中断 PC+4（3 项）。全量 **738 项检查 0 失败**。

## 2026-09-06 · 21-B9y — IRQ 返回恢复 IRQ SP 用例
- `[case 21-B9c]` 再补：handler 返回并由 `irq_hle_restore` 恢复后，IRQ 私有
  SP 回到压帧前值（1 项）。全量 **739 项检查 0 失败**。

## 2026-09-06 · 21-B9z — B7 地址掩码与尾部零填充用例
- 15.2/15.4/15.5 假 ROM 改为 0x10000，B7 地址使用 0x8100（>=0x8000 不触发
  重定向），并断言 4 字节块读完后再读返回 0xFFFFFFFF。全量 **739 项检查 0 失败**。

## 2026-09-06 · 21-B9za — GXSTAT FIFO IRQ 用例
- 19.5 用例补：写 0x04000603=0x80 后 GXSTAT bits30-31=0x80000000，
  ARM9 IF bit21 置位；写 0 后清除。全量 **742 项检查 0 失败**。

## 2026-09-06 · 21-B9zg — ARM7 STM 基址在列表内新增 4 项
...
## 2026-09-06 · 21-B9zo — 卡带 B7 块字段按 melonDS 重映射

- 15.2/15.4/15.5 的 B7 激活从块字段 0 改为 1：melonDS 的 1 表示 0x200，
  0 表示无数据；旧用例依赖本地“0=默认 0x200”的错误扩展。全量 **751 项
  检查 0 失败**。

## 2026-09-06 · 21-B9vv — 周期成本骨架新增 4 项

- `[case 21-B9vv]`：WFI 无中断时 step_cycles=0 且指令计数不增；普通指令
  step_cycles=1 且指令计数+1。全量 **755 项检查 0 失败**。

## 2026-09-06 · 21-B9vw — 事件表新增 7 项
...
## 2026-09-06 · 21-B9wh — CLZ 非零 Rd 回归新增 3 项

- `[case 21-B9d]` 补：`E16FAF13`（CLZ r10,r3，Rd≠0）执行后 r10=1、CPSR
  保持 0x8000001F 不被 CMN 误写、PC 前进 4。旧掩码 0x0FFFFFF0 只匹配
  Rd=0，此用例在修复前会把 r10 留在旧值并改标志。
- 全量从 **792 项**增至 **795 项检查，0 项失败**。

## 2026-09-06 · 21-B9wi — ARM7 音频/启动 SWI 查表新增 10 项

- 新增 `[case 21-B9wi]`：Thumb SWI 0x1A/0x1B/0x1C 分别对正弦索引 1、
  音高索引 1、音量索引 0x2A0 返回 FreeBIOS 表值，PC 前进 2；SWI 0x1D
  返回 r0=0xA2E/r1=0x2C3C/r2=0x5FF。
- 全量从 **795 项**增至 **805 项检查，0 项失败**。

## 2026-09-06 · 21-B9wj — VRAM 直显模式新增 3 项

- 新增 `[case 21-B9wj]`：LCDC 窗口 bank0 写红/蓝、bank1 写绿后，把主引擎
  DISPCNT 设为 `2<<16` 直显 bank0，断言前两像素红/蓝；再切 bit18-19=1
  （bank1）断言首像素绿。验证渲染器确实按物理 bank 而不是 2D 图层取图。
- 全量从 **805 项**增至 **808 项检查，0 项失败**。

## 2026-09-06 · 21-B9wk — ARMv5 DSP 乘法新增 10 项

- 新增 `[case 21-B9wk]`：SMULBB/SMULBT/SMULTB 半字组合、SMLABB 加数与
  溢出 Q(bit27)、SMLALBB 64 位累加、SMULWy 32×16；并用 ARM7 执行
  `E15100CC`（CMP_REG，row 0x15）验证不会误入 DSP 未定义路径。
- 全量从 **808 项**增至 **818 项检查，0 项失败**。

## 2026-09-06 · 21-B9wo — 定时器按周期推进（API 变化，无新增用例计数）

- `timer_advance/io_advance_timers` 增加 `cycles` 参数；`[case 6.5]` 的
  TM0 1:1（640 步=640 计数）、TM1 1:64（10 计数）与
  `[case 21-B9j]` TM3 0xFFF0 溢出/reload 用例全部保持原有期望值通过。
- 全量保持 **818 项检查，0 项失败**。

## 2026-09-06 · 21-B9wq — -O2 构建回归（无新增用例计数）

- CMake 给 ndscore/nds-emu/test_nds 加 `-O2` 后重跑全量：仍为
  **818 项检查，0 项失败**；新增 `--headless-frames` 属 runner 集成路径，
  由真 ROM 帧驱动长跑覆盖（frame1700 与旧 step 驱动结果一致）。

## 2026-09-12 · 21-B9wr — KEYCNT 与 NDS7 卡带 DMA（新增 12 项）

- `[case 6.7]` 补 4 项：KEYCNT OR 模式（释放不触发、按下触发 IF bit12）、
  AND 模式（掩码内部分按下不触发、全部按下触发）。
- 新增 `[case 21-B9wr]` 8 项：以 ARM7 视角写 DMA 寄存器（落到 `dma[1]`），
...
## 2026-09-12 · 21-B9wu — DMA 归属与无头 SPU 推进（新增 5 项）

- 新增 `[case 21-B9wu]` 3 项：配置 ARM9 的 DMA0（源 RAM → 目的 GXFIFO、
  VBlank 触发），把 `bus->active_is_arm7` 故意污染成 ARM7 后触发搬运，断言
  GX 命令数 +2、`mt_mode` 被写成参数值（说明命令确实进了 GX 而不是音频）、
  搬运结束后访问者标志原样恢复。
- 新增 `[case 21-B9wu]` 2 项：PCM8 单发通道置 start 后调用
  `snd_advance(2048)`，断言 start 位被硬件清掉（无头模式游戏轮询播放状态
  依赖这条路径）。
- 全量从 **860 项**增至 **865 项检查，0 项失败**。

## 2026-09-12 · 21-B9wv — KEYINPUT 默认值（新增 1 项）

- `[case 6.7]` 增加断言：`nds_create()` 之后直接读 KEYINPUT 必须是
  `0xF000 | 0x0FFF`（全松开）。旧实现读 0（= 全按下），游戏会误判 START/A 一直按住。
- 全量从 **865 项**增至 **866 项检查，0 项失败**。

## 2026-09-12 · 21-B9ww/21-B9wx — GX FIFO DMA 与 RTC（新增 9 项）

- `[case 21-B9ww]` 3 项：ARM9 DMA 模式 7（源=主存、目的=GXFIFO 固定）触发后
  GX 命令数 +2、`mt_mode` 写成参数值、使能位自动清零。
- `[case 21-B9wx]` 6 项：RTC 串行协议往返（写状态寄存器 1=24 小时制 → 读回、
  读 7 字节日期时间并校验 BCD 年/月/日、RCnt 高低字节可读写）。
- 全量从 **866 项**增至 **875 项检查，0 项失败**。

## 2026-09-12 · 21-B9wy — GXSTAT FIFO 状态位（新增 4 项）

- `[case 21-B9wy]`：`nds_create()` 后读 `0x04000600`，断言 bit25（不足半满）
  与 bit26（空）都在；写 `0x04000603=0x80`（FIFO IRQ 模式 2）后断言模式位
  置上且 FIFO 状态位仍在（与参考核 `0x86000000` 一致）。
- 全量从 **875 项**增至 **879 项检查，0 项失败**。

## 2026-09-12 · 21-B9xa — IME 掩码（无新增用例计数）
...
## 2026-09-12 · 21-B9xj — DISPSTAT 每核一套（新增 9 项）

- `[case 21-B9xj]`：ARM7 写 `0x04000004` 只改它自己那份（ARM9 读回仍 0）；
  只读位 0-2/6 写不进去（写 `0xFFFF` 后低字节只剩 `0x00B8`）；
  VCount 匹配是边沿触发：命中时置标志，只有使能位（bit5）才挂 IF bit2，
  且**只挂给该寄存器所属的核**（ARM7 那份挂 IF[1]，ARM9 那份挂 IF[0]）。
- 全量从 **894 项**增至 **903 项检查，0 项失败**。

## 2026-09-12 · 21-B9xq — GBA 扩展槽空槽 open bus（新增 4 项）

- `[case 21-B9xq]`：`0x08000000-0x0BFFFFFF`（slot-2 窗口）无卡时 8/16/32 位读
  都应返回 `0xFF` 填充（`0xFF` / `0xFFFF` / `0xFFFFFFFF`），ARM7 视角同样成立。
- 全量从 **903 项**增至 **907 项检查，0 项失败**。

## 2026-09-12 · 21-B9yc — 颜色期望随 RGB555→888 换算更新（28 条改写）

- 背景：`rgb555_to_888` 对齐参考核的 5bit→6bit→8bit 路径后，白 31 由 `0xF8`
  变为 `0xFB`，旧期望值编码的是旧换算，因此先跑一次拿到 **28 条失败清单**。
- 这 28 条**全部是纯色期望**（RGB555 直接展开），无 CPU 符号扩展
  （`bhw LDRSB`/`LDRSH`、`thumb NEG`）与混合/亮度类（`alpha blend`、
  `bright up/down`、`master bright`）混入 ⇒ 可精确按字面量替换：
...
## 2026-09-12 · 21-B9yg — 帧驱动时钟折算（单测不变，附替代验证）

- 本步改的是 `src/runner/runner.c`（帧驱动）里 `cost9` 到系统时钟的折算
  （`/2` → `/1`），**单测不覆盖 runner**，因此全量仍是 **921 项检查 0 项失败**；
- 替代验证写进 [runnerlog](../src/runner/runnerlog.md) 与
...
## 2026-09-12 · 21-B9yi（续）— 卡带中断语义（930 项）

- `[case 15.4]` 卡带 DMA 用例改写：数据就绪（DRQ）**只触发 DMA、不挂中断**
  （`card no irq on DRQ`）；打开 `AUXSPICNT bit14` 并把本块读完 →
  传输结束挂卡带中断（`card irq on transfer end`）。
- 背景：旧期望编码的是「每个字挂一次 IF bit19」的旧实现，实测会让 ARM9
  每 ~170 周期被中断一次并在第 1900 帧死锁；melonDS 只在传输结束挂中断。
- 全量从 **929 项**增至 **930 项检查，0 项失败**。

## 2026-09-12 · 21-B9yi（续）— 卡带 DMA 修复（930 项保持全绿）

- 修复 ①重复模式地址不前进（搬完把 `src/dst` 写回通道，卡带 DMA 逐字搬
  512 字节时目标地址继续前进）；②`dma_fire_card()` 增加 DRQ 门控
  （对齐 melonDS `CheckDMA()` 的 `if (!(ROMCnt & (1<<23))) return;`）。
- 用例侧同步：`card dma word[]` 改为「逐字武装 count=1 + 推进卡带时钟」的真实
  用法（游戏代码就是 `AF000001` 模式 5 + 重复位逐字推进）；
  `card prog word[]` / `card7 dma word[]` 在 CPU 程序停机后显式推进卡带时钟，
  让重复触发的 DMA 把 4 个字搬完（此前依赖的是一次性搬完整块的旧行为）。
- 全量保持 **930 项检查，0 项失败**；900 帧里程碑标记与参考核一致。

## 2026-09-12 · 21-B9yi（续12）— 卡带 DMA 用例改成真机口径（931 项 0 失败）

`dma_fire_card()` 改成参考核的**电平敏感**模型（每个 DRQ 一次 `CheckDMA`、
`CNT_L=1` + 重复位逐字搬）后，三个卡带 DMA 用例的**旧期望**不再成立——旧的
「一次武装多字、搬完自动清使能」本身就不符合真机节奏。改动：

| 用例 | 旧期望 | 新期望（真机口径） |
|---|---|---|
| `[case 15.4] card dma` | 逐字武装 count=1；搬完 DRQ 仍置位 | 读走一个字后 DRQ **落 0**（`ReadROMData` 清 bit23），推进卡带时钟取到下一字才回到 1（新增 `card romctrl DRQ refilled`） |
| `[case 21-B9wr] card7`（ARM7 模式 0x12） | 一次武装 4 字 | 武装 `CNT_L=1` + **重复位**（`A700`），循环推进卡带时钟让 4 字依次落下；断言重复位下使能**保持**（不再自动清） |
| `[case 15.5] card prog` | CPU 程序一次武装 4 字（`AD000004`，无重复位） | CPU 程序改写成游戏同款 `**AF000001**`（+重复位）；断言使能保持（这正是游戏把整段 512 字节交给 DMA 逐字搬完的机制） |

全量 **931 项检查，0 项失败**（新增 1 项）；900 帧里程碑三标记与参考核一致；
帧 100 画面 81.9% 不回归。（带按键时序的 1904 帧停摆仍在，机理见
`docs/21-rom-bringup.md` 续12 第 5 节。）

## 2026-09-12 · 21-B9yi（续13）— 「无传输时的完成中断」修复（931 项保持全绿）

- 根因：`cartbus_read32()` 在**没有传输**时（`xfer_len==0`）也能走到
  `pos>=len`（0>=0）分支 ⇒ 调 `cartbus_end()` ⇒ 多挂一次 IF bit19；
  游戏任务计数每个完成中断减 `0x200`，多一次就提前一块以为搬完。
- 修正：`cartbus_end()` 只在 `xfer_len != 0`（有传输在跑）时挂完成中断。
- 单测 **931 项 0 失败** 保持；900 帧里程碑三标记与参考核一致；
  带按键时序下 f=1910 起 ARM9 已回到空闲任务（不再死等 ROMCTRL），
  游戏走完加载阶段并开始出三角形。

## 2026-09-12 · 21-B9yi（续32/33）：GX 条目流重构后的用例核对

- **做了什么**：GX 队列从「每次端口写 = 一条完整命令」改成 melonDS 的条目流
  （一次写 = 一个 `{Command, Param}` 条目）后，重跑全部用例并逐条核对
  受影响的 5 项期望值（`dma gx cmds`、`gxfifo dma cmds`、`gx fifo in`…）。
- **怎么验证**：`build\test_nds.exe`（`ctest` 同一入口）。
- **结果**：✅ **931 项检查 0 项失败**。
  语义澄清：melonDS 里 0 字节条目是 NOP（无副作用），本地不再把它计入
  `cmd_count`/命令直方图，因此「4 字 DMA 里 2 条真命令」的期望值 2 仍然成立。

## 2026-09-12 · 21-B9yi（续34）：视口映射口径更正后的用例核对

- **做了什么**：视口映射改为 melonDS 口径（DS 的 `VIEWPORT` 里 y0/y1 是「0=底」，
  屏幕行号 = `191-y1` 起、高 `y1-y0+1`；并把右下边界钳到屏内）后，
  逐条核对了受影响的 `test_gx_transform` 期望值。
- **怎么验证**：`build\test_nds.exe`。
- **结果**：✅ **931 项检查 0 项失败**。变更点：屏幕中心由 (127,95) 更正为 (128,96)
  （旧值是「(vx2-vx1)/2」的近似，且整幅 3D 画面还额外垂直镜像）。

## 2026-09-12 · 21-B9yi（续35）：VTX_10/VTX_DIFF 缩放修正后重跑

- **做了什么**：`VTX_10`/`VTX_DIFF` 的 10 位坐标改为按 melonDS 口径「符号扩展后原值」
  （此前错误地 `<<2`），并新增视体裁剪、矩阵栈重写、`MTX_SCALE` 只缩放前 3 行。
- **怎么验证**：`build\test_nds.exe`（含用 `gx_raster_tri`/`gx_emit_prims` 的
  三角形与 FIFO 用例）。
...
## 2026-09-13 · 21-B9yi（续45）：4bpp 图块格式更正后的用例核对

- **做了什么**：NDS 2D 的 **16 色图块是「半字节打包线性」**（32 字节/图块、4 字节/行、低半字节在左），
  本地此前按 **GBA 位平面**解码。修正 `tile_pixel()` 后逐条核对了 3 处「测试数据按旧格式构造」
  的用例并改写（`tile4`、`engB tile`、`obj`、`scene obj`：像素(0,0)=索引1 从 `0x80` 改成 `0x01`；
  「全索引1」从 `0xFF` 改成 `0x11`）。
- **怎么验证**：`build\test_nds.exe`。
- **结果**：✅ **931 项检查 0 项失败**；f=3000/6000 两屏统计与修正前逐值相同（无回归）。

## 2026-09-13 · 21-B9yi（续108）：ARM9 BIOS 区读数用例

- **背景**：跨核逐字节对账发现，本地对 ARM9 BIOS 区（0xFFFF0000-0xFFFF3FFF）一律读 0，
  而参考核（FreeBIOS）在 `SetupDirectBoot()` 里把卡带头的任天堂 logo 拷进 BIOS 偏移 0x20，
  FFXII 会把那 156 字节读回去用（`memcpy(0x020798A4, 0xFFFF0020, 0x9C)`）。
- **新增用例（`test_bios9_image()`，6 项）**：注入 logo 后 `bus_read32(0xFFFF0020)` 等于
  logo 首字（小端拼装）；logo 最后一个字节可读；logo **之后**的偏移仍是 FreeBIOS 字节
  （注入前后一致，说明没有越界覆盖）；注入确实改变了读回值；BIOS **只读**（写 0xDEADBEEF
  后读回不变）；**ARM7 视角读不到**高地址 BIOS（未映射读 0）。
- **怎么验证**：`build\test_nds.exe`。
- **结果**：✅ **982 项检查 0 项失败**（976 → 982）。

## 2026-09-13 · 21-B9yi（续109/109b）：主存 16MB 窗口 + Thumb BLX 字对齐

- **背景**：Phase C 用第二款商业 ROM（口袋妖怪 黑2）做泛化验收，本地起不来，
  参考核正常 ⇒ 反推出两处真实缺陷。
- **新增用例（+9 项）**：
  1. 主存窗口（`test_main_ram_mirror()` 扩写 5 项）：`0x023FFC24` 写 → `0x02FFFC24`
     读到同一字节（双核握手的实际地址）；反向写 `0x02FFFC26` → 主存可见；
     4MB 周期镜像 `0x02A01234` ↔ `0x02201234`；**ARM7 视角**同口径；
     窗口末字节 `0x02FFFFFF` 仍是主存顶字节。原「0x02800000 读 0」的越界断言
     按新口径删除（该地址现在**就是**主存偏移 0）。
  2. Thumb BLX（4 项）：`0xF000,0xE80F` ⇒ 原始和 = `base+0x22`（bit1=1），
     断言 PC = `base+0x20`（字对齐）、LR = `(base+6)|1`、T=0；
     既有的 `0xE80E`（本来字对齐）用例保留做对照。
- **怎么验证**：`build\test_nds.exe`。
- **结果**：✅ **997 项检查 0 项失败**（989 → 994 → 997）。

## 2026-09-13 · 21-B9yi（续109c）：Thumb BL/BLX 返回地址改回规范值

- **背景**：续109b 之后第二款 ROM 卡在 IPCSYNC 等待循环（指针 r0=0 —— 循环前那条
  `subs r0,r5,#4` 被跳过）。trace 显示 BL 返回后 PC = BL 地址+6、LR = +7。
- **改动**：`thumb.c` BL/BLX 两处 `cpu->r[14] = (pc | 1u)` → `((pc - 2u) | 1u)`
  （= 「BL 之后那条指令」| 1，与 melonDS `T_BL_LONG_2` 的 `(R[15]-2)|1` 一致）。
- **测试期望值更正（重要）**：`thumb BL LR` / `thumb BLX LR` / `thumb BLX odd LR`
  三条断言原为 `(base + 6) | 1` —— 那是把**旧实现的行为**当成了期望值；按规范
  （返回 = BL 之后那条指令 = base+4）改为 `(base + 4) | 1`。
- **怎么验证**：`build\test_nds.exe`。
- **结果**：✅ **997 项检查 0 项失败**；FFXII 锚点 2000 帧统计/截图哈希/savechip
  三项逐值不变（说明该修复对 FFXII 无副作用）；第二款 ROM 的死锁症状消失。

## 2026-09-13 · 21-B9yi（续109d）：Thumb BLX Rm 的 LR + 6 项新用例

- **改动**：`thumb.c` 里 `BLX Rm`（`4790` 系列）的 LR 由 `pc | 1`（= 后缀地址 +5）
  改为 `(pc - 2) | 1`（= 下一条指令 | 1），与 melonDS `T_BLX_REG`（`lr = R[15]-1`）
  及本轮已修的 BL/BLX 立即数口径一致。
- **新增用例（+6 项，2 组）**：
  1. `BLX r2`（`0x4790`）r2 = `(base+0x40)|1`（奇数）⇒ LR = `(base+2)|1`、T=1、
     PC = base+0x40；
  2. 同上但 r2 = base+0x40（偶数）⇒ LR 同样 `(base+2)|1`、T=0、PC = base+0x40。
- **怎么验证**：`build\test_nds.exe`。
- **结果**：✅ **1003 项检查 0 项失败**（997 → 1003）；FFXII 锚点 2000 帧三项逐值不变。

## 2026-09-13 · 21-B9yi（续109e/109f/109g）：Thumb 读 PC、互调加载、ITCM 窗口

- **新增用例（+15 项）**：
  1. Thumb 高位寄存器形式（3 项）：`ADD r0,PC`(0x4478) ⇒ r0 = base+4；`MOV r0,PC`(0x4678)
     同；`ADD PC,r0`(0x4487) ⇒ PC = base+4+Rm。
  2. ARMv5T 互调加载（5 项）：ARM9 `LDR pc,[r0]` 载入奇数 ⇒ T=1、PC 清 bit0；
     ARM9 `LDMIA r0,{pc}` 同；ARM7 同指令 ⇒ T 保持 0（ARMv4T 不互调）。
  3. ITCM 窗口（6 项）：`bus_set_arm9_itcm(1,0,32MB)` 后 `0x0080` 与 `0x01FF8080`
     互为别名、`0x0100` 写读同字节；ARM7 看不到 ITCM；窗口搬回 `0x01FF8000` 后
     `0x0080` 不再命中；关闭使能后整窗读 0。
- **怎么验证**：`build\test_nds.exe`。
- **结果**：✅ **1018 项检查 0 项失败**（1003 → 1006 → 1012 → 1018）。
