...
（同时把该桶最后命中的**真实 PC** 记进 `RefPcLast`），harness 退出前打印前 16 名。
分桶与本模拟器 `NDS_PCHOT=1` 完全一致，于是「同一段游戏进度、两边各在跑哪些代码」
可以直接逐行对照。

> **口径更正（21-B9yi 续108q）**：最早一版只打印 `best << 4`（= 桶起点，只保留 PC 的
> bit4..19），于是 ARM9 的 `0x020119xx` 被打成 `0x000119xx`，一度被读成「参考核跑在
> 主存镜像区」。加上 `bucket=` + `pc=`（`RefPcLast`）后结论变得很干净。

**结论（f=50、同键脚本、精确 PC）**：两边 ARM9 的热点是**同一批地址** ——

```text
本地 hot9（精确 PC）          参考核 refhot9（精确 PC）
...
⇒ 开机段（f≤50）的指令数差（本地 +20%）**不是「跑了不同的代码」**，而是**同一个循环里
两边的指令占比/计费不同**（本地 `1191`/`1193` 两个桶更集中），与
`NDS_INSTRSTAT`/`NDS_MEMTIM`（续108e/g）的结论一致。

**ARM7 侧**：参考核第一热点 `0x037FB82C`（6.1%）、`0x037FB81C`（3.1%）与本地
`0x037FB92C`/`0x037FBD2C`（0.9%/0.6%）都落在 **ARM7 WRAM / 共享 WRAM 里的中断等待与
派发 helper**（反汇编 `shared_wram 0x38F4-0x393C`：`ldr r1,[ip,#-8]` → 逐个测 IF 位
→ `bx` 到 handler 表）；两边第一热点差 `0x100`（0x82C vs 0x92C）说明**同一段 helper 的
入口偏移/命中位置不同**，属「等待点不同」而不是「代码不同」。另外两边都出现
`0x023801xx` 这种主存热点，而该地址在 **f=1 是 ARM7 镜像**（`01 C3 A0 E3 08 C2 8C E5 …`）、
**f=2 已被清零**（参考核 f=50 主存同址同样是 0）⇒ 这类高占比来自**开机头两帧**，
两边行为一致。
